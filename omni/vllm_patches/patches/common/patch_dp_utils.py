...
def _get_dp_sync_primitives() -> (
    ...
    if _dp_sync_group_key == group_key:
        ...
        if any(item is None for item in primitives):
            raise RuntimeError("DP sync primitives are uninitialized")
        return primitives
    ...
    try:
        ...
        if device_group is None:
            raise RuntimeError("failed to create AICPU DP sync process group")
...
def _synchronize_dp_ranks(
    ...
    if num_tokens_padded < num_tokens_unpadded:
        raise RuntimeError("num_tokens_padded must be >= num_tokens_unpadded")
