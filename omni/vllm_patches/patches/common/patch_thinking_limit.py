...
def _reasoning_config_from_env() -> ReasoningConfig | None:
    ...
    if not raw:
        return None
    try:
        cfg = _coerce_reasoning_config(raw)
    except Exception as exc:  # noqa: BLE001
        logger.error(
            "Error parsing %s environment variable: %s", _REASONING_CONFIG_ENV, exc
        )
        return None
    if cfg is not None:
        logger.info(
            "Loaded reasoning_config from the %s environment variable", _REASONING_CONFIG_ENV
        )
...
def _get_request_thinking_token_budget(
    ...
    return _normalize_thinking_token_budget(
        getattr(request, _THINKING_TOKEN_BUDGET_KEY, None)
    )
...
class EngineArgsPatch(VLLMPatch):
    ...
    def create_engine_config(
        ...
        reasoning_config_env = _reasoning_config_from_env()
        if reasoning_config_env:
            reasoning_config = reasoning_config_env
...
def _register_thinking_token_budget_field(model_cls: type) -> None:
    if _THINKING_TOKEN_BUDGET_KEY in model_cls.model_fields:
        return
    model_cls.__annotations__[_THINKING_TOKEN_BUDGET_KEY] = int | None
    model_cls.model_fields = {
        **model_cls.model_fields,
        _THINKING_TOKEN_BUDGET_KEY: Field(default=None),
    }
    model_cls.field_names = None
    model_cls.model_rebuild(force=True)

"""
clear alarm:WARNING [protocol.py:117] The following fields were present in the request 
but ignored: {'thinking_token_budget'}
"""
_register_thinking_token_budget_field(ChatCompletionRequest)
    ...
    def apply_logits_processors(
        ...
        if holder is not None and holder.has_tracked_requests():
            if repeat_indices is None:
                raise RuntimeError("repeat_indices is None")
            logits = holder.apply_to_logits(
                logits,
