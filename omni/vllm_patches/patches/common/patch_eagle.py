...
class EagleProposerPatch(VLLMPatch):
    ...
    def load_model(self, target_model: nn.Module) -> None:
        ...
        if supports_multimodal(target_model):
            ...
            if not hasattr(target_model, "config"):
                raise RuntimeError("multimodal target model must have config")
    ...
    def propose(
        ...
        if self.method in ("eagle3", "dflash"):
            ...
            if not isinstance(draft_model, allowed_draft_models):
                raise TypeError(
                    "draft_model must be one of "
                    f"{allowed_draft_models}, got {type(draft_model)}"
                )
        ...
        for token_index in range(self.num_speculative_tokens - 1):
            ...
            if draft_probs is not None:
                if draft_probs_list is None:
                    raise RuntimeError("draft_probs_list must not be None")
