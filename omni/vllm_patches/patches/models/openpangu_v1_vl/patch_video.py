...
import cv2
import itertools
# vllm 0.25.1: the old monolithic `OpenCVVideoBackend` class was removed.
# The built-in `openpangu` backend now uses `(VideoLoader,
# OpenCVVideoBackendMixin)` as its base. Mirror that here so our NPU dynamic
# backend stays compatible. This backend fully overrides `load_bytes`, so it
# does not depend on the removed class's sampling orchestration.
...
class NPUOpenCVDynamicVideoBackend(VideoLoader, OpenCVVideoBackendMixin):
    @staticmethod
    ...
    def load_bytes(
        ...
        **kwargs,
        ...
        try:
            ...
            if total_frames_num <= 0 or original_fps <= 0:
                raise ValueError("Video must have a positive frame count and FPS")
