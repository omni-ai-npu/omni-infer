...
class PanguV2HybridGPUModelRunnerPatch(VLLMPatch):
    ...
    def _build_attention_metadata(
        ...
        assert num_reqs_padded is not None and num_tokens_padded is not None, (
            "num_reqs_padded and num_tokens_padded must be resolved before building metadata."
        )
        ...
        def _get_block_table_and_slot_mapping(kv_cache_gid: int):
            assert num_reqs_padded is not None and num_tokens_padded is not None, (
                "num_reqs_padded and num_tokens_padded must be resolved before block mapping."
            )
        ...
        def _build_attn_group_metadata(
            ...
            if ubid is None:
                assert isinstance(attn_metadata, dict), (
                    "attn_metadata should be a dict when ubid is None."
                )
                attn_metadata_dict = attn_metadata
            else:
                assert isinstance(attn_metadata, list), (
                    "attn_metadata should be a list when ubid is provided."
                )
