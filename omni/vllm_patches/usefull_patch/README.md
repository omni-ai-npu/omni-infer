...
| `patch_mrv2_sampler.py` | MRv2 | V2 采样算子 `gumbel_sample` / `apply_top_k_top_p` / `rejection_sample` 换 NPU 实现或绑定（上游随机数、`log1p`、`None` optional tensor 和 top-k/top-p Triton kernel 在 triton-ascend 3.2.2 上不适配）。只换 V2 消费方，MRv1 用的定义模块 `v1/sample/ops/topk_topp_sampler.py` 不动 |
