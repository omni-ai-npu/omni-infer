...
class W8A8Int8FCLinearMethod(FlashCommLinearMethodBase):
    ...
    def apply(
        ...
        if isinstance(x, Dict):
            ...
            if x_transform not in ("DP2TP", "DP2TPAll2All"):  # dynamic quant after communication
        ...
        elif x_transform == "DP2TP":
            x = layer_parallel_dp2tp_single(
                x, layer.layer_name_inside_block, "x", x_dim
            )
            if x_scale is None:
                x, x_scale = torch_npu.npu_dynamic_quant(x)
                ...
                x_scale = layer_parallel_all_gather(
                    x_scale, layer.layer_name_inside_block, "x", 0
                )
        elif x_transform == "DP2TPAll2All":
            x = layer_parallel_dp2tp_all2all(
                x, layer.layer_name_inside_block, "x", x_dim
            )
            if x_scale is None:
                x, x_scale = torch_npu.npu_dynamic_quant(x)
