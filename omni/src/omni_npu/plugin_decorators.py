...
def attn_decorator(attn_type: str = None, **kwargs):
    """
    ...
    Args:
        attn_type: Attention type, e.g., 'dsa', 'mla', 'mome', 'compress_mqa', 'mqa'
        **kwargs: Supports legacy type='...' call sites for backward compatibility.
    ...
    if kwargs:
        ...
        raise TypeError(f"attn_decorator() got an unexpected keyword argument '{unexpected}'")
    if attn_type is None:
        attn_type = legacy_attn_type
    elif legacy_attn_type is not None and legacy_attn_type != attn_type:
        raise ValueError("attn_type and type must match when both are provided")
    if attn_type is None:
