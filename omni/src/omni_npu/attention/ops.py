...
def apply_FA_rescale_forward(
    output: torch.Tensor,              # [T, N, D] BF16
    softmax_max: torch.Tensor,         # [T, N, 1] FP32
    softmax_sum: torch.Tensor,         # [T, N, 1] FP32
    output_sink: torch.Tensor,         # [T, N, D] BF16
    softmax_max_sink: torch.Tensor,    # [T, N, 1] FP32
    softmax_sum_sink: torch.Tensor,    # [T, N, 1] FP32
):
    # use max between two softmax max to normalize
    softmax_max_full = torch.maximum(softmax_max, softmax_max_sink)
    softmax_sum_orig_add_max = softmax_sum * torch.exp(
        softmax_max - softmax_max_full
    )
    softmax_sum_sink_add_max = softmax_sum_sink * torch.exp(
        softmax_max_sink - softmax_max_full
    )
    softmax_sum_full = softmax_sum_orig_add_max + softmax_sum_sink_add_max
    # Guard div-by-zero / NaN (e.g. fully masked query: max=-inf, sum=0).
    # Valid path keeps the original division; do not add eps (precision-neutral).
    valid = softmax_sum_full > 0
    zero = torch.zeros_like(softmax_sum_full)
    rescale_orig = torch.where(
        valid, softmax_sum_orig_add_max / softmax_sum_full, zero
    )
    rescale_sink = torch.where(
        valid, softmax_sum_sink_add_max / softmax_sum_full, zero
    )
    core_attn_out = output * rescale_orig + output_sink * rescale_sink
    core_attn_out = core_attn_out.to(dtype=output.dtype)
    # shapes: [T, N, D], [T, N, 1], [T, N, 1]
    # dtypes: BF16, FP32, FP32
    return core_attn_out, rescale_orig, rescale_sink
