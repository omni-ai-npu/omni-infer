...
class PanguReasoningParser(DeepSeekR1ReasoningParser):
    ...
    def _should_implicit_end_in_delta(
        self,
        tool_start_id: int | None,
        delta_token_ids: Sequence[int],
        previous_token_ids: Sequence[int],
    ) -> bool:
        if tool_start_id is None:
            return False
        if tool_start_id not in delta_token_ids:
            return False
        if self.end_token_id in delta_token_ids:
            return False
        return self.end_token_id not in previous_token_ids
    ...
    def extract_reasoning_streaming(
        ...
        if self._should_implicit_end_in_delta(
            tool_start_id,
            delta_token_ids,
            previous_token_ids,
        ):
