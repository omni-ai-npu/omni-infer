...
class ColumnParallelMOME(torch.nn.Module):
    ...
    def forward(
        ...
        if self.on_ascend950:
            # The fused kernel now handles prefill / decode / mixed scenarios
            # internally from the metadata (run_mode is a no-op), so a single
            # call replaces the old forward_prefill / forward_decode split.
            conv_kwargs = {
