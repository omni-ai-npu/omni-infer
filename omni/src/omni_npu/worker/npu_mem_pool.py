...
def find_loaded_library(lib_name) -> Optional[str]:
    ...
    lib_basename = filename.rpartition(".so")[0]
    if not lib_basename.startswith(lib_name):
        raise ValueError(
...
try:
    from omni_npu.allocator.npu_mem_allocator import (init_module, python_create_and_map,
...
class NpuMemAllocator:
    ...
    def get_instance() -> "NpuMemAllocator":
        ...
        """
        if not npu_mem_available:
            raise RuntimeError("npu_mem_available is not available")
