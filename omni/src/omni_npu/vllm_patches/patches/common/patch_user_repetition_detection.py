...
def _repetition_config_from_env() -> RepetitionDetectionParams | None:
    ...
    if not raw:
        return None
    try:
        cfg = _coerce_repetition_detection(raw)
    except Exception as exc:  # noqa: BLE001
        logger.error(
            "Error parsing %s environment variable: %s", _REPETITION_CONFIG_ENV, exc
        )
        return None
    if cfg is not None:
        logger.info(
            "Loaded repetition detection config from the %s environment variable", _REPETITION_CONFIG_ENV
        )
...
class EngineArgsPatch(VLLMPatch):
    ...
    def create_engine_config(
        ...
        repetition_detection_env = _repetition_config_from_env()
        if repetition_detection_env:
            repetition_detection = repetition_detection_env
        
        vllm_config.repetition_detection = repetition_detection
        return vllm_config
...
def _apply_init_bypassing_conflict(cls: type) -> None:
    """Apply ``cls``'s attributes to its target without touching the shared
    ``_omni_npu_applied_patches`` dict.

    ``OpenAIServingChat`` and ``OpenAIServingCompletion`` are sibling
    subclasses of ``OpenAIServing``. Once any patch targets the base
    ``OpenAIServing`` through the default ``VLLMPatch.apply()``, both
    siblings inherit the *same* ``_omni_npu_applied_patches`` dict, so
    registering ``__init__`` on one sibling makes the default ``apply()``
    reject the other with ``"... already patched by ..."``. Bypass the dict
    here, mirroring ``OpenAIServingLoptPatch.apply``.
    """
    target = cls._target
    ...
    logger.info(
        "patch applied: %s => %s (bypass-conflict)", cls.__name__, target.__name__
    )
...
class OpenAIServingChatRepetitionPatch(VLLMPatch):
    ...
    @classmethod
    def apply(cls):
        _apply_init_bypassing_conflict(cls)
...
def check_sequence_repetition(
    ...
    if total_tokens < min_pattern_size * min_count:
        return False

    last_token = token_ids[-1]
    max_possible_len = min(max_pattern_size, total_tokens // min_count)

    for pattern_len in range(min_pattern_size, max_possible_len + 1):
        if token_ids[-1 - pattern_len] != last_token:
            continue

        total_check_len = pattern_len * min_count
        tail = token_ids[-total_check_len:]
        
        # 周期错位对比
        if tail[:-pattern_len] == tail[pattern_len:]:
            return True
