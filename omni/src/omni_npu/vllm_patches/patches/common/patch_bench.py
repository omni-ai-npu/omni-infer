...
from vllm.benchmarks.datasets import (
    SampleRequest, add_dataset_parser, get_samples
)
from vllm.benchmarks.serve import (
    BenchmarkMetrics, TaskType, calculate_metrics,
    calculate_metrics_for_embeddings, get_request,
    check_goodput_args, save_to_pytorch_benchmark_format
)
            ...
            with open(self.dataset_path, 'r', encoding='utf-8') as file:
                ...
                for item in json_data:
                    ...
                    if item["prompt"] is None:
                        raise ValueError(
                            "Each item in the JSON list must be a "
                            "dictionary with a 'input' key or a 'prompt' key."
                        )
        ...
        if self.custom_data_type == "jsonl":
            ...
            for i in range(num_requests):
                ...
                if output_len is not None:
                    ...
                    raise ValueError(
                        f"output_len of prompt: {prompt} is None, "
                        "add output_len in json file or specify via "
                        "--fixed-output-len."
                    )
...
class BenchServePatch(VLLMPatch):
    ...
    async def main_async(args: argparse.Namespace) -> dict[str, Any]:
        ...
        if not match:
            raise ValueError(
                "Invalid --range-metrics format. Expected format: 'start%,end%', " \
                "start and end must be integers <= 100 and >=0, with a < b.")
        else:
        ...
        try:
            ASYNC_REQUEST_FUNCS["openai-chat"] = sys.modules[
        ...
        if not (
            ...
            raise ValueError("multi_modal_data must be a dict or list[dict]")
        ...
        if len(input_requests) % args.num_processes != 0:
            remaining = input_requests[args.num_processes * chunk_size:]
            sub_input_requests_list[-1].extend(remaining)
        ...
        if args.save_result or args.append_result:
            ...
            if args.ramp_up_strategy is not None:
                file_name = f"{label}-ramp-up-{args.ramp_up_strategy}-{args.ramp_up_start_rps}qps-"
                f"{args.ramp_up_end_rps}qps{max_concurrency_str}-{base_model_id}-{current_dt}.json"  # noqa
            else:
                file_name = f"{label}-{args.request_rate}qps{max_concurrency_str}-"
                f"{base_model_id}-{current_dt}.json"  # noqa
