...
including 3D patch embedding, RoPE position embedding, and temporal
pooling for video chunks.
"""
...
def is_vit_use_data_parallel() -> bool:
    try:
...
def _apply_rope_input_validation(x, freqs_cis):
    assert x.ndim in (freqs_cis.ndim, freqs_cis.ndim + 1), (x.shape, freqs_cis.shape)
    if x.ndim == freqs_cis.ndim:
        assert x.shape[:-1] == freqs_cis.shape[:-1], (x.shape, freqs_cis.shape)
    else:
...
def _get_rope_shape(org: torch.Tensor, interpolation_mode: str, shape: tuple[int, int]):
    return (
        F.interpolate(
