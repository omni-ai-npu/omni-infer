...
class MoMEPatch(VLLMPatch):
    ...
    class AggregateConv(CustomOp):
        def __init__(
            ...
            super().__init__()
            # TODO: fix Cannot run aclop operators during NPU graph capture. Current working aclop is Conv2D
            import torch_npu
            ...
            self.cache_states = torch.zeros((self.max_num_seqs + 1, self.cache_length, hidden_size), device=current_platform.device_type)
            ...
            self.cache_states = torch.zeros((self.max_num_seqs + 1, self.cache_capacity, hidden_size), device='npu')

        def cache_lmove(self, cache_slot_id: torch.Tensor, target_indices: torch.Tensor):
            tmp = torch.gather(self.cache_states[1:], 1, target_indices.unsqueeze(-1).expand(-1, -1, self.hidden_size))
            cache_slot_id_t = torch.arange(cache_slot_id.shape[0],device=cache_slot_id.device,dtype=cache_slot_id.dtype).unsqueeze(1) + 1
            torch_npu.npu_scatter_nd_update_(self.cache_states, cache_slot_id_t, tmp)
        ...
        def forward(self, hidden_states: torch.Tensor, only_prefill=False, force_decode=False, short_prefill=False) -> torch.Tensor:
            ...
            if (attn_metadata.num_prefills > 0 or batch_descriptor.num_tokens > attn_metadata.num_decodes or short_prefill) and not force_decode:
                ...
                for i in range(batch_size):
                    ...
                    conv_input = torch.cat([self.cache_states[cache_slot_id[i], :self.cache_length], local_input], dim=0)
                    ...
                    if not self.padding and cache_slot_id[i] == 0:
                        ...
                        self.cache_states[i + 1, :self.cache_capacity, :] = conv_input[-self.cache_capacity:, :]
                ...
                conv_input = torch.cat(
                    [
                        self.cache_states[
                            cache_slot_id[:num_tokens], : self.cache_length
                        ],
