...
class PanguV2HybridForCausalLMConfig(MambaModelConfig):
    ...
    def verify_and_update_config(cls, vllm_config: "VllmConfig") -> None:
        ...
        num_speculative_tokens = (
            0
            if not vllm_config.speculative_config
        ...
        dsa_page_dim = None
        if (
            hasattr(model_config.hf_config, "index_topk")
            and model_config.hf_config.index_topk > 0
        ):
        ...
        mome_state_shapes = (
            ...
            (
                ...
                * model_config.hf_config.v_head_dim,
            ),
        )
        mome_state_dtypes = (
            torch.bfloat16,
        ...
        if dsa_page_dim is not None:
            ...
            if mome_page_size is not None:
                assert target_page_size >= mome_page_size, (
                    f"block size {block_size} * dsa page dim {dsa_page_dim} "
                    f"should be greater than mome page size {mome_page_size}."
                )
...
class PanguV2HybridVllmConfigPatch(VLLMPatch):
    ...
    def try_verify_and_update_config(self):
        ...
        cls = models_config_module.MODELS_CONFIG_MAP.get(architecture, None)
        already_updated = getattr(self.model_config, "config_updated", False)
        needs_hybrid_page_align = (
            already_updated
            and cls is PanguV2HybridForCausalLMConfig
            and self.cache_config.mamba_page_size_padded is None
        )

        # Avoid running try_verify_and_update_config multiple times, but still
        # allow this patch to fill hybrid page-size alignment if an earlier
        # config pass marked the model as updated before this patch was applied.
        if already_updated and not needs_hybrid_page_align:
            return
        ...
        if hasattr(self.model_config, "model_weights") and is_runai_obj_uri(
            ...
            if self.load_config.load_format == "auto":
                ...
                raise ValueError(
                    ...
                    "must be 'runai_streamer' or 'runai_streamer_sharded', "
