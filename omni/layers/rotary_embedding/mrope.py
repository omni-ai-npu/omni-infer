    ...
    def forward_oot(
        ...
        if is_prefill:
            if positions.ndim not in (1, 2):
                raise ValueError("positions must be a 1D or 2D tensor.")
            if key is None:
                raise ValueError("key must be provided for prefill rotary embedding.")
            ...
            if attn_metadata is not None and hasattr(attn_metadata, '_npu_cached_cos_sin'):
                ...
                if positions.ndim == 2:
                    if not self.mrope_section:
                        raise ValueError("mrope_section must be set for 2D positions.")
