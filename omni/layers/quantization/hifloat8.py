...
class Hifloat8FCLinearMethod(FlashCommLinearMethodBase):
    ...
    def create_weights(self, layer, input_size_per_partition, output_partition_sizes, input_size, output_size, params_dtype, **extra_weight_attrs):
        ...
        # FlashComm weight_loaders squeeze loaded weights unless is_2_dims is set,
        # which would collapse the (out, 1) channel scale to (out,) and fail the
        # shape assert. Both params are genuinely 2-D; mark them so the loader
        # skips the squeeze, matching the plain Hifloat8LinearMethod path.
        set_weight_attrs(weight, {"is_2_dims": True})
    ...
    def apply(self, layer, x, bias=None, x_transform=None, x_dim=0, throw_dequant=False):
        ...
        # Cube-side overlap (opaque to Dynamo): fire the task registered under
        # layer_key on the cube-side stream now, mirroring Hifloat8LinearMethod.
        # Keeps the A5 HiF8 MHC sinkhorn side task firing for o_proj after it was
        # swapped to a FlashComm* linear (no-op for q_b_proj/kv_b_proj, which
        # register no task).
        x = torch.ops.vllm.cube_side_run(layer_key, x)
...
class Hifloat8MoEMethod(FusedMoEMethodBase, NPUFusedMoEMethodBase):
    ...
    def apply_experts(
        ...
        if run_shared_with_cv:
            shared_expert_act = activate_shared_expert_on_side_stream(
                self.shared_experts_stream, layer, prepare_permute_result,
                lambda act: torch_npu.npu_dtype_cast(act, torch_npu.hifloat8),
            )
