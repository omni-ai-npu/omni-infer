...
class NPURoutedExperts(RoutedExperts):
    """omni-aware RoutedExperts.

    Registered as the OOT ``RoutedExperts`` so every MoE layer gets it, including
    models that call vLLM's ``FusedMoE`` factory directly (the factory picks the
    OOT class via ``PluggableLayer.__new__``, exactly like ``NPUFusedMoERunner``
    is picked for ``MoERunner``).

    vLLM moved expert weight loading onto RoutedExperts, and binds every
    expert param's ``weight_loader`` to ``RoutedExperts.weight_loader`` at
    ``create_weights`` time. That native name-based dispatcher misroutes W4A8
    aux params:

    * ``weight_bias`` (1D per expert) matches the ``"weight"`` case and is
      treated as a matrix weight, crashing in ``_load_w2`` on ``shape[1]``.
    * ``weight_int4_scale`` / ``weight_offset`` land in the per-channel branch
      but with ``shard_dim`` derived from ``SHARD_ID_TO_SHARDED_DIM`` (w1->0),
      which is wrong for the W4A8 ``(E, 1, C)`` layout whose channel dim is the
      last dim -- nothing gets loaded and the scale stays at its ``ones`` init.

    ``weight_bias`` is fully recomputed from packed+scale in
    ``process_weights_after_loading`` (so its loaded value is irrelevant, it
    only must not crash); ``int4_scale`` / ``weight_offset`` feed that recompute
    and the GMM directly, so they must be loaded with the correct shard dim.

    Override ``weight_loader`` to route those aux params to
    ``_load_per_channel_weight_scale`` with a shard dim derived from the
    actual per-expert layout (the channel dim = the largest non-expert dim), so
    it is correct for both the W4A8 ``(1, C)`` and W8A8 ``(C, 1)`` layouts.
    Everything else delegates to the native loader, which already handles
    packed matrix weights and W8A8 ``weight_scale`` correctly.
    """
    ...
    def weight_loader(
        ...
        if not any(
            name in weight_name
            for name in ("weight_bias", "int4_scale", "weight_offset")
        ):
        ...
        if quant_method != FusedMoeWeightScaleSupported.CHANNEL.value:
            raise ValueError(
                f"quant method must be "
                f"{FusedMoeWeightScaleSupported.CHANNEL.value}, {weight_name=}"
            )
        ...
        # Channel dim = the largest non-expert dim: W4A8 aux is (1, C) (channel
        # last), W8A8 offset is (C, 1) (channel first), weight_bias is (C,) 1D.
        # w2 ignores shard_dim for sharding (full copy), so this only matters
        # for w13, where it must point at the 2I channel dim.
        candidate_dims = range(1, expert_data.ndim) if full_load else range(expert_data.ndim)
        shard_dim = max(candidate_dims, key=lambda d: expert_data.shape[d])
    ...
    # Mark the override as MoE-loading capable, mirroring the native loader.
    weight_loader.supports_moe_loading = True  # type: ignore[attr-defined]
