...
def _fused_recurrent_gated_delta_rule_ref(
    ...
    contiguous_states = ssm_state_indices is not None
    ...
    q = q * scale
    g = g.exp()
    #Now q & k have shape [B, T, HV, K], and v & o have shape [B, T, HV, V]
    ...
    o = torch.empty_like(v) # [A, T, bs, C, E]  
    ...
    for t in range(T):
        ...
        if not contiguous_states:
            initial_state[ssm_state_indices[indices_0 + t]] = S.to(torch.bfloat16)
        else:
            if eq_len:
                indices = cu_seqlens[:-1] + t
                initial_state[indices] = S.to(torch.bfloat16)
            else:   
                initial_state[indices_0 + t] = S.to(torch.bfloat16)
        o[0][t] = o_t.to(torch.bfloat16)
    o = o.transpose(1, 2).reshape(A, tbs, C, E)
