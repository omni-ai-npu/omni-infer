...
from omni.layers.moe.deepseek_moe import DeepseekMoE as Glm4MoE
...
class Glm4MoeMultiTokenPredictorLayer(nn.Module):
    ...
    def compute_logits(
        ...
        logits = self.logits_processor(
            self.shared_head["head"], hidden_states, sampling_metadata
        )
        return logits

    def should_use_eager_mode(self, *args, **kwargs) -> bool:
        if len(kwargs) == 0:
            return True
