...
`template/` 保存重构前的历史部署脚本，仅供追溯原流程。该目录后续不继续维护，
不能作为新增部署或修复问题的入口。
...
│       ├── defaults/main.yml
│       └── tasks/                  # 扩缩容生命周期
└── template/                       # 历史部署归档，后续不维护
```

从 `tools/ansible` 目录执行命令。`ansible.cfg` 已将 `roles_path` 设置为
...
`tools/ansible/template/` 中的旧文件不在这条运行链路中，仅用于追溯重构前的
部署实现。
...
从 `tools/ansible` 目录执行时，Ansible 会自动读取本目录的 `ansible.cfg`：
...
输出中的 `config file` 应指向 `tools/ansible/ansible.cfg`，
`DEFAULT_ROLES_PATH` 应包含 `tools/ansible/roles`。
...
```bash
ANSIBLE_ROLES_PATH=/path/to/project/roles:/absolute/path/to/omniinfer/tools/ansible/roles \
  ansible-playbook -i /path/to/inventory.yml /path/to/model.yml
```
...
```bash
ANSIBLE_CONFIG=/absolute/path/to/omniinfer/tools/ansible/ansible.cfg \
  ansible-playbook -i /path/to/inventory.yml /path/to/model.yml
```
...
开发者也可以从熟悉的 `template/` 历史脚本开始迁移，但它使用旧的任务组织方式。
最终提交的 Playbook 应改用当前公开 Profile 和 Role 入口；历史脚本中的特殊任务
按第 10 节迁移到专用 Role。
...
旧 CI 或长期测试模板缺少 `CODE_PATH`、`LOG_PATH_IN_EXECUTOR` 时，迁移到默认
...
### 5.1 当前配置与原始 Inventory 文件

仓库不提交面向当前 roles 的具体 Inventory。用户应参考本节字段结构，在仓库外
维护实际 `inventory.yml`，避免把生产环境地址和凭据带入代码提交。仓库内现有
Inventory 均为重构前的原始环境文件，只用于追溯旧部署环境和拓扑，不会继续维护，
也不能因为文件名相近就直接用于重构后的 playbook。

| 文件 | 用途和兼容性 |
| --- | --- |
| `omni_infer_inventory_used_for_CI.yml` | 原始 CI 环境 Inventory，使用 `npu_server_list`、节点内私钥路径等旧配置，不能直接用于当前 roles。 |
| `omni_infer_inventory_user_for_long_term_test.yml` | 原始长期测试环境 Inventory，结构与旧 CI Inventory 接近，包含特定环境地址和路径，仅用于追溯。 |
| `template/omni_infer_inventory_a5.yml` | A5 历史部署使用的 P/D/C Inventory，与不再迁移的 A5 模板配套。 |
| `template/omni_infer_inventory_used_for_1P1D.yml` | 历史 1P1D 拓扑基线，与 `template/` 中原始部署脚本配套。 |
| `template/omni_infer_inventory_used_for_2P1D.yml` | 历史 2P1D 扩容拓扑，用于对比旧流程中的节点扩容配置。 |
| `template/omni_infer_inventory_used_for_4P1D.yml` | 历史 4P1D 拓扑，保留旧流程的多 Prefill 节点配置。 |
| `template/omni_infer_inventory_used_for_4P1D_a3_1machine2p.yml` | A3 单机双 Prefill 分组拓扑，包含 PA/PB 特殊分组，不属于当前迁移范围。 |
...
| `workdir` | `{{ container_workspace }}/omniinfer/tools/scripts` | 容器内启动脚本目录。 |
...
默认不绑核。启用后，Role 等待 Decode 服务就绪，再执行容器内
`tools/scripts/bind_cpu.sh`。该脚本路径由 `container_workspace` 决定，不跟随
Prefill/Decode `workdir`。
...
默认列表为空，执行删除时会校验 IPv4。建议通过 `-e` 临时传入，不要把生产 IP
提交到 playbook。执行前必须先从 Inventory 的 P/D 分组中移除目标节点，使 Proxy
按删除后的拓扑刷新；目标 IP 仍须能使用 `all.vars` 中的 SSH 配置连接。
...
| `sync_code` | rsync 到远端，并按需复制进容器。 | `host_sync.delete=true` 会删除目标端多余文件。 |
