#!/bin/bash
# One 8-card A2 node -> 4 x TP2 vLLM -> 1 proxy port

# PATH
MOUNT_PATH="${MOUNT_PATH:-/home/ma-user/work}"
BUCKET_PATH="${BUCKET_PATH:-bucket-pangu-green-guiyang}"
BASE_LOG_PATH="${BASE_LOG_PATH:-${MOUNT_PATH}/${BUCKET_PATH}/omni-elb/logs/gpt-oss-20b}"
MODEL_PATH="${MODEL_PATH:-${MOUNT_PATH}/${BUCKET_PATH}/omni-elb/models/GPT/gpt-oss-20b-int8/}"
...
BASE_API_PORT="${BASE_API_PORT:-8000}"
...
MAX_NUM_SEQS="${MAX_NUM_SEQS:-192}"
COMPILATION_CONFIG="{\"level\": 3, \"cudagraph_mode\":\"FULL_DECODE_ONLY\", \"cudagraph_capture_sizes\":[${MAX_NUM_SEQS}], \"backend\":\"eager\", \"compile_sizes\":[${MAX_NUM_SEQS}]}"
