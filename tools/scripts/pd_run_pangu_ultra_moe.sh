...
#OMNI_MOCK_RANDOM_MODE=0
#OMNI_MOCK_KV_CACHE_MODE=0
#OMNI_MOCK_FORWARD_TIME=0
...
print_help() {
    ...
    echo "  --random-mode                    mockModel configuration: Enable mock model when set to 1 (default: $OMNI_MOCK_RANDOM_MODE)"
    echo "  --kv-cache-mod                   mockModel configuration: Enable mock model when set to 1 (default: $OMNI_MOCK_KV_CACHE_MODE)"
    echo "  --forward-time                   mockModel configuration: Forward time in ms (default: $OMNI_MOCK_FORWARD_TIME)"
...
parse_long_option() {
    case "$1" in
        ...
        --use-inventory-devices)
            ...
            ;;
        --random-mode)
            OMNI_MOCK_RANDOM_MODE="$2"
            ;;
        --kv-cache-mod)
            OMNI_MOCK_KV_CACHE_MODE="$2"
            ;;
        --forward-time)
            OMNI_MOCK_FORWARD_TIME="$2"
            ;;
        --num-servers)
            NUM_SERVERS="$2"
            ;;
...
#export OMNI_MOCK_RANDOM_MODE
#export OMNI_MOCK_KV_CACHE_MODE
#export OMNI_MOCK_FORWARD_TIME
...
echo "OMNI_MOCK_RANDOM_MODE: $OMNI_MOCK_RANDOM_MODE"
echo "OMNI_MOCK_KV_CACHE_MODE: $OMNI_MOCK_KV_CACHE_MODE"
echo "OMNI_MOCK_FORWARD_TIME: $OMNI_MOCK_FORWARD_TIME"
...
# Regenerate the UB endpoint configs the hixl backend links through. Gated on the same switch the
# consumer reads, so platforms without UB (A2/A3) skip it. Done here rather than in the orchestrator
# so every launch path gets it, and on every launch, since a device reallocation would otherwise
# leave stale EIDs behind.
case "$(echo "${HIXL_LOCAL_COMM_RES_ENABLE:-false}" | tr '[:upper:]' '[:lower:]')" in
  ...
  *)
    ...
    # Default to /etc/hixlep, but fall back to a per-user directory where that is not writable
    # (containers running as a non-root user). Exported either way, so the workers that read this
    # same variable inherit whichever path was actually used.
    ...
    if ! mkdir -p "$HIXLP_ENDPOINT_PATH"; then
        echo "ERROR: cannot create $HIXLP_ENDPOINT_PATH, point HIXLP_ENDPOINT_PATH at a writable directory" >&2
        exit 1
    fi
    gen_ep_cmd=(python "$SCRIPT_DIR/generate_ep_server_pod.py" --pod)
    # Serialise P and D when they share one container; skip the lock where flock is unavailable
    if command -v flock >/dev/null 2>&1; then
        gen_ep_cmd=(flock "$HIXLP_ENDPOINT_PATH/.generate.lock" "${gen_ep_cmd[@]}")
    fi
    if ! "${gen_ep_cmd[@]}"; then
        echo "ERROR: failed to generate endpoint configs in $HIXLP_ENDPOINT_PATH, the hixl link would fail at runtime" >&2
        exit 1
    fi
    ;;
esac
