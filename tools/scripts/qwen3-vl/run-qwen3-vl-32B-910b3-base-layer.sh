...
VLLM_PLUGINS="omni-npu,omni_npu_patches" vllm serve "$MODEL_PATH" \
  ...
  --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY", "cudagraph_capture_sizes":[32], "backend":"eager", "compile_sizes":[32]}' 2>&1 | tee -a "$LOG_FILE"