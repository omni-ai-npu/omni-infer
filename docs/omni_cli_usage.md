...
# 配置管理
omni_cli cfg
omni_cli inspect    # 当前版本执行会报 config_path 错误，暂不可用
...
# 开发工具
omni_cli upgrade     # 当前版本执行会报 config_path 错误，暂不可用
omni_cli collect_logs
...
## 1. omni_cli cfg（建议手动修改yaml配置文件效率更高，不需要用cfg命令修改）
...
## 2. omni_cli inspect（v1版本下该命令暂时不可用，可直接看yaml文件，P/D/C节点的拉起情况直接查询相应日志即可`LOG_PATH`）
...
> --help: 帮助信息
> --skip-verify-config: 跳过配置校验
> --config_path: 使用指定配置文件
> --run_dev: 开发者快速拉起
...
## 2. omni_cli collect_logs （原v0命令是collect_log不可用）
...
```bash
omni_cli collect_logs --deploy_path server_profiles.yml
```
...
功能：
将指定路径源代码同步到所有节点对应的容器中的 `/workspace/` 目录下。

> 实际同步目录为 `${code_path}/omniinfer/`，并通过 rsync 同步到各节点宿主机，再通过 docker cp 拷贝到容器中。

---

当前版本行为：

* 实际同步的是 `${code_path}/omniinfer/`
* 不会同步 `${code_path}` 根目录本身
* 不会同步 `${code_path}` 下除 `omniinfer/` 以外的其他目录（例如 components、tools 等），除非这些目录本身位于 `${code_path}/omniinfer/` 内
* 容器内最终路径为：
...
* `omni_cli sync_code --code_path /data/workspace/omniinfer`
  
  实际同步的是：
  
  ```
  /data/workspace/omniinfer/omniinfer/
  ```
* 若希望同步整个仓库根目录（包含 components、tools 等），应使用：
  
  ```
  omni_cli sync_code --code_path /data/workspace
  ```
...
* `--deploy_path`：配置文件路径，默认为当前执行目录下的 `server_profiles.yml`
...
```bash
omni_cli sync_code --code_path /path/to/my/code
# 实际同步 /path/to/my/code/omniinfer/
```

若希望同步整个 omniinfer 仓库根目录：

```bash
omni_cli sync_code --code_path /path/to
# 实际同步 /path/to/omniinfer/
```
---
