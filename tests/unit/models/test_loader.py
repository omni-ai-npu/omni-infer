...
class TestConfigLoaderUnit(unittest.TestCase):
    ...
    # ------------------------------------------------------------------
    # _match_model_name: multi-match disambiguation (lines 272-278)
    # ------------------------------------------------------------------
    ...
    def test_match_model_name_multiple_matches_deepseek_v3(self):
        ...
        self.assertEqual(result, "deepseek_v3")
    ...
    def test_match_model_name_multiple_matches_deepseek_v32(self):
        ...
        self.assertEqual(result, "deepseek_v32")
    ...
    def test_match_model_name_multiple_matches_unknown_raises(self):
        ...
        with patch(
            ...
            with self.assertRaises(RuntimeError) as ctx:
                _match_model_name(hf_config, vars(hf_config))

        self.assertIn("Multiple matching model names found", str(ctx.exception))

    # ------------------------------------------------------------------
    # _extract_quantization_config: text_config fallback (line 287)
    # ------------------------------------------------------------------
    ...
    def test_extract_quantization_config_via_text_config(self):
        ...
        # Ensure no quantization_config attr on hf_config itself
        self.assertFalse(hasattr(hf_config, "quantization_config"))

        result = _extract_quantization_config(hf_config)
    ...
    # ------------------------------------------------------------------
    # _resolve_int_quant_type: dict branches (lines 295, 299, 304, 306-307)
    # ------------------------------------------------------------------
    ...
    def test_resolve_int_quant_type_dict_weights_and_activations(self):
        ...
        quantization_config = {
            "config_groups": {
                "group_0": {
                    "weights": {"num_bits": {"layer1": 8, "layer2": 4}},
                    "input_activations": {"num_bits": {"a": 8, "b": 4}},
                }
            },
            "kv_cache_scheme": "default",
        }

        result = _resolve_int_quant_type(quantization_config)
        # min(4, 8) for both → w4a4, default scheme → c16
        self.assertEqual(result, "w4a4c16")
    ...
    def test_resolve_int_quant_type_opti_c8_scheme(self):
        ...
        quantization_config = {
            ...
            "kv_cache_scheme": "Opti-C8",
        }

        result = _resolve_int_quant_type(quantization_config)
        self.assertEqual(result, "w8a8_fa_c8")
    ...
    def test_resolve_int_quant_type_dict_kv_cache_scheme(self):
        ...
        result = _resolve_int_quant_type(quantization_config)
        self.assertEqual(result, "w8a8c8")

    # ------------------------------------------------------------------
    # _resolve_quant_type: hifloat8 / mxfp8 branches (lines 318-322)
    # ------------------------------------------------------------------
    ...
    def test_resolve_quant_type_mxfp8(self):
        ...
        result = _resolve_quant_type(hf_config)
        self.assertEqual(result, "mxfp8")

    # ------------------------------------------------------------------
    # _select_config_file: decode branch (line 410)
    # ------------------------------------------------------------------
    ...
    def test_select_config_file_decode_node(self):
        ...
        files_data = {
            ...
            "decode_config_file": "d.json",
        }
        task_config = TaskConfig(is_pd_disaggregation=True, is_prefill_node=False)
        result = _select_config_file(files_data, task_config)
        self.assertEqual(result, "d.json")
