...
@pytest.fixture()
def caplog_config_summary(capture_logger):
    """Capture config_summary logs even when omni_npu propagation is disabled.

    Thin wrapper over the shared ``capture_logger`` fixture (conftest.py), pinned
    to the config_summary logger so tests can just request this fixture and read
    ``.records``.
    """
    with capture_logger(logging.getLogger(_CONFIG_SUMMARY_LOGGER)) as caplog:
        yield caplog
...
def test_ast_audit_omni_custom_configs():
    ...
    src_root = pathlib.Path(__file__).parents[4] / "omni"
    unclassified = []
...
class TestCollectors:
    ...
    def test_collect_omni_walks_loaded_config(self, monkeypatch):
        ...
        fake_loader = types.ModuleType(
            "omni_npu.model_config.config_loader.loader"
        )
        fake_loader.model_extra_config = MEC()
        fake_mod.loader = fake_loader
        monkeypatch.setitem(
