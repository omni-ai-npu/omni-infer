...
def _stub_fused_moe_deps(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _ensure_module(monkeypatch, "torch_npu.npu_fusion_attention")
    ...
    import vllm  # pre-load real submodules into sys.modules before stubbing  # noqa: E402
    import vllm.config
    ...
    # Keep the real vllm.utils (pre-loaded by `import vllm` above) —
    # just stub random_uuid on top of it.
    monkeypatch.setattr(sys.modules["vllm.utils"], "random_uuid", lambda: "stub-uuid")
    ...
    vllm_module.utils = sys.modules["vllm.utils"]
    vllm_module.config = sys.modules["vllm.config"]
