...
def test_all2all_prepare_permute_mxfp8_routes_bf16_then_quants_sorted(prepare_module):
    ...
    quant_args, quant_kwargs = stubs.torch_npu.npu_dynamic_mx_quant.call_args
    assert quant_args[0] is sorted_states
    assert quant_kwargs["dst_type"] is torch.float8_e4m3fn
    assert result.hidden_states_sorted_by_experts is sorted_fp8
    assert result.dynamic_scale is sorted_scale
...
def test_allreduce_prepare_permute_no_quant(prepare_module):
    ...
    expanded_x = torch.ones(4, 2, dtype=torch.float32)
    expanded_row_idx = torch.tensor([0, 1, 2, 3], dtype=torch.int32)
    expert_tokens = torch.tensor([1, 0, 0, 1], dtype=torch.int32)
    stubs.torch_npu.npu_moe_init_routing_v2.return_value = (
        expanded_x,
        expanded_row_idx,
        expert_tokens,
    ...
    args, kwargs = stubs.torch_npu.npu_moe_init_routing_v2.call_args
    assert args[0].shape == (4, 2)
    assert args[1].dtype == torch.int32
    assert kwargs["active_num"] == topk_ids.numel()
    assert kwargs["expert_num"] == layer.w13_weight.shape[0] * stubs.ep_group.world_size
    assert kwargs["active_expert_range"] == [2, 4]
    assert kwargs["row_idx_type"] == 0
    assert torch.equal(result.hidden_states_sorted_by_experts, expanded_x)
    assert torch.equal(result.expert_tokens, expert_tokens)
    assert torch.equal(result.expanded_row_idx, expanded_row_idx)
    assert result.expert_range == [2, 4]
...
def test_agrs_prepare_permute_cv_mxfp8_quants_on_side_stream(prepare_module, monkeypatch):
    ...
    local_n = 2
    x = torch.zeros(local_n, 8, dtype=torch.float32)
    topk_ids = torch.zeros(local_n, 1, dtype=torch.int32)
    expanded_x_bf16 = torch.zeros(local_n, 8, dtype=torch.float32)
    expanded_row_idx = torch.tensor([0, 1], dtype=torch.int32)
    expert_tokens = torch.tensor([1, 1], dtype=torch.int32)
    # Side-stream mx-quant must run on the LOCAL (pre-allgather) tokens
    # because shared experts are replicated across EP ranks.
    side_fp8 = torch.zeros(local_n, 8, dtype=torch.int8)
    side_scale = torch.zeros(local_n, 8 // 32 + 1, dtype=torch.uint8)
    post_fp8 = torch.ones(local_n, 8, dtype=torch.int8)
    post_scale = torch.ones(local_n, 8 // 32 + 1, dtype=torch.uint8)
    ...
    # First call (side stream) must receive the LOCAL bf16 tensor — not the
    # ep_size×local_N gathered tensor. Regression guard for the EP>1 shape
    # mismatch where shared_output had ep_size×local_N rows.
    side_call_input = stubs.torch_npu.npu_dynamic_mx_quant.call_args_list[0].args[0]
    assert side_call_input.shape[0] == local_n
...
def test_all2all_unpermute_finalize_precision_strong_consistency(prepare_module, monkeypatch):
    module, stubs = prepare_module
    monkeypatch.setattr(
        module.model_extra_config.operator_opt_config,
    ...
    assert args[0].dtype == topk_weights.dtype
    assert kwargs["scales"].dtype == topk_weights.dtype
    assert kwargs["drop_pad_mode"] == 2
...
def test_allreduce_unpermute_finalize_uses_saved_routing(prepare_module):
    ...
    prepare_result = module.AllReducePreparePermuteResult(
        hidden_states_sorted_by_experts=torch.zeros(2, 2),
        expert_tokens=torch.tensor([1, 1], dtype=torch.int32),
        dynamic_scale=None,
        expanded_row_idx=torch.tensor([1, 0], dtype=torch.int32),
        topk_ids=torch.tensor([[3], [2]], dtype=torch.int32),
    )
    hidden_states = torch.ones(2, 2, dtype=torch.float16)
    topk_weights = torch.ones(2, 1, dtype=torch.float32)
    ...
    output = handler.unpermute_finalize(
        ...
        topk_ids=torch.zeros(2, 1, dtype=torch.int32),
        topk_weights=topk_weights,
        allreduce_prepare_permute_result=prepare_result,
    )

    assert torch.equal(output, torch.full((2, 2), 9.0, dtype=torch.float16))
    args, kwargs = stubs.torch_npu.npu_moe_finalize_routing.call_args
    assert torch.equal(args[0], hidden_states.unsqueeze(0))
    assert kwargs["skip1"] is None
    assert kwargs["skip2"] is None
    assert kwargs["bias"] is None
    assert kwargs["scales"].dtype == hidden_states.dtype
    assert torch.equal(kwargs["expanded_src_to_dst_row"], prepare_result.expanded_row_idx)
    assert torch.equal(kwargs["export_for_source_row"], prepare_result.topk_ids)
...
def test_dispatch_prepare_permute_mxfp8_disables_dispatch_quant(prepare_module):
    ...
    stubs.context_holder.attn_metadata = {
        0: SimpleNamespace(decode=SimpleNamespace(mc2_mask=torch.tensor([1, 1], dtype=torch.bool)))
    }
    expand_x = torch.ones(2, 3)
    dirty_dynamic_scale = torch.ones(2)
    expand_idx = torch.zeros(2, dtype=torch.int32)
    expert_token_nums = torch.tensor([1, 1], dtype=torch.int32)
    recv_counts = torch.tensor([1, 1], dtype=torch.int32)
    ...
    assert result.hidden_states_sorted_by_experts is expand_x
    assert result.dynamic_scale is None
    assert result.expert_tokens.dtype == torch.int64
...
def test_agrs_unpermute_finalize_quant_row_idx_type_1_uses_grouped_finalize(
    ...
    assert torch.allclose(kwargs["logit"], torch.tensor([0.2, 0.8, 0.7, 0.7]))
...
def test_strategy_selector_allreduce_requires_dp_one(prepare_module):
    ...
    module.model_extra_config = SimpleNamespace(
        operator_opt_config=SimpleNamespace(
            decode_moe_dispatch_combine=False,
            enable_moe_agrs=False,
            enable_moe_allreduce=True,
        ),
        parall_config=SimpleNamespace(ena_seq_parallel=False),
    )

    with pytest.raises(AssertionError, match="dp_size should == 1"):
        module.CommunicationStrategySelector(layer)
...
def test_strategy_selector_allreduce_overrides_strategy(prepare_module):
    ...
    selector = module.CommunicationStrategySelector(layer)
    strategy, strategy_impl = selector.select_communication_strategy(num_tokens=4)

    assert strategy == "allreduce"
    assert isinstance(strategy_impl, module.AllReducePrepPmtAndUnpmtFinal)
...
def test_prepare_finalize_params_returns_none_no_quant(prepare_module):
    ...
    result = handler.prepare_finalize_params(
        ...
        agrs_prepare_permute_result=module.AGRSPreparePermuteResult(
            ...
            dynamic_scale=None,
            expert_range=[0, 4],
            expanded_row_idx=torch.zeros(1, dtype=torch.int32),
            gathered_topk_ids=torch.zeros(1, 2, dtype=torch.int32),
            dtype=torch.float16,
            row_idx_type=1,
        ),
    )
...
def test_unpermute_finalize_with_finalize_params(prepare_module, monkeypatch):
    ...
    handler = module.AGRSPrepPmtAndUnpmtFinal(
        SimpleNamespace(global_num_experts=4, w13_weight=torch.zeros(2, 1, 1), top_k=1)
    )
    handler.batch_size = 2
...
def test_agrs_prepare_permute_hifloat8_with_routed_experts_cv_runs_shared_gate_up(
    ...
    on AGRSPreparePermuteResult.shared_expert_finished_event."""
    module, stubs = prepare_module
    module.model_extra_config = SimpleNamespace(
        operator_opt_config=SimpleNamespace(
    ...
    # Side-stream must feed the LOCAL (pre-allgather) tensor — not the
    # ep_size×local_N gathered one. Regression guard for the EP>1 shape
    # mismatch on `final_hidden_states + shared_output` in pangu_v2_moe.py.
    assert arg["x_hif8"].shape[0] == local_n
    ...
    assert result.shared_expert_finished_event is recorded_events[1]
...
def test_agrs_prepare_permute_default_finalize_no_shared_gate_up(prepare_module):
    ...
    assert result.shared_expert_finished_event is None
