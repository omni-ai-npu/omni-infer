...
@pytest.mark.unit
def test_apply_and_maybe_mome_q_kv_paths():
    from omni_npu.v1.layers.attention.npu_mla import NPUDeepseekMLAAttention

    fake = NPUDeepseekMLAAttention.__new__(NPUDeepseekMLAAttention)
    q = torch.ones(2, 4)
    fake.use_mome = False
    assert fake._maybe_mome_q(q, lambda: {}) is q

    fake.use_mome = True
    fake.noncontiguous_kv = True
    fake.conv = MagicMock(return_value=q + 1)
    out = fake._maybe_mome_q(q, lambda: {"scale": 1})
    torch.testing.assert_close(out, q + 1)
    out = fake._apply_mome(q, 0, lambda: {"scale": 1})
    fake.conv.assert_called_once()
    torch.testing.assert_close(out, q + 1)

    fake.noncontiguous_kv = False
    conv0 = MagicMock(return_value=torch.ones(2, 4))
    fake.qa_conv = conv0
    fake.compresskv_conv = MagicMock()
    fake.o_conv = MagicMock()
    out = fake._apply_mome(q, 0, lambda: {})
    torch.testing.assert_close(out, q + torch.ones(2, 4))

    fake.kv_lora_rank = 2
    fake.qk_rope_head_dim = 2
    kv = torch.arange(8, dtype=torch.float32).reshape(2, 4)
    fake._apply_mome = lambda x, idx, get_args: x + 1
    out_kv = fake._maybe_mome_kv(kv, lambda: {})
    assert out_kv.shape == kv.shape
    torch.testing.assert_close(out_kv[:, :2], kv[:, :2] + 1)
    torch.testing.assert_close(out_kv[:, 2:], kv[:, 2:])

    fake.use_mome = False
    assert fake._maybe_mome_kv(kv, lambda: {}) is kv


@pytest.mark.unit
def test_register_sink_params_and_sink_kv_weight_loader(monkeypatch):
    from omni_npu.v1.layers.attention import npu_mla as mla_mod
    from omni_npu.v1.layers.attention.npu_mla import NPUDeepseekMLAAttention

    loaded = {}

    def fake_load(module, param, weight):
        loaded["called"] = (module, param, weight)
        param.data.copy_(weight)

    monkeypatch.setattr(
        "omni_npu.v1.layers.attention.weight_utils.load_sharded_param_weight",
        fake_load,
    )
    monkeypatch.setattr(mla_mod, "set_weight_attrs", lambda *a, **k: None)

    fake = NPUDeepseekMLAAttention.__new__(NPUDeepseekMLAAttention)
    fake.param_sink_number = 2
    fake.qk_rope_head_dim = 4
    fake.kv_lora_rank = 3
    fake.default_cfg = {"device": "cpu", "dtype": torch.float32}
    fake._register_sink_params(SimpleNamespace())
    assert tuple(fake.param_sink_k_pe.shape) == (2, 4)
    assert tuple(fake.param_sink_compressed_kv.shape) == (2, 3)

    weight = torch.ones(2, 4)
    fake.sink_kv_weight_loader(fake.param_sink_k_pe, weight)
    assert loaded["called"][0] is fake
    torch.testing.assert_close(fake.param_sink_k_pe.data, weight)
