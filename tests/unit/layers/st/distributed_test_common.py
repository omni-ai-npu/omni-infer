...
def _destroy_layer_parallel_ext_groups() -> None:
    try:
        ...
        for name in ("_LOCAL_WORLD",):
            ...
            if group is not None:
                try:
                    ...
                    pass
                setattr(pse, name, None)

        for name in ("_LOCAL_COMM_LIST", "_CROSS_COMM_LIST",
                     "_CROSS_ROUND_COMM_LIST"):
            groups = getattr(pse, name, None)
        ...
        layer_comm = getattr(pse, "_LAYER_COMM_DICT", None)
        if layer_comm:
            for layer_cfg in layer_comm.values():
                for item in layer_cfg.values():
                    group = item.get("parallel_group") if isinstance(item, dict) else item
                    if group is not None:
                        try:
                            group.destroy()
                        except Exception:
                            pass
        pse._LAYER_COMM_DICT = None
    except Exception:
        pass

def _destroy_distributed_groups() -> None:
    _destroy_layer_parallel_ext_groups()
    try:
...
def _persistent_worker_loop(device: int, rank: int, world_size: int, temp_file_path: str, 
    ...
    try:
        ...
        with set_current_vllm_config(vllm_config):
            ...
            if enable_expert_parallel and getattr(
                vllm_dist.parallel_state, "_MOE_DISPATCH_EP", None
            ) is None:
                vllm_dist.parallel_state._MOE_DISPATCH_EP = (
                    vllm_dist.parallel_state.get_ep_group()
                )
