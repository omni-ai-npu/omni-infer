...
Covers the pure SSE-line helpers ``_maybe_split_sse_line`` and
``_rewrite_named_finish_reason_sse``. The patch class itself
(``OpenAIServingChatStreamSplitPatch``) is a thin async-generator
wrapper around those helpers — separately exercising the wrapper would
require driving the full ``chat_completion_stream_generator`` chain,
which is heavier than the regression value justifies.
"""
...
class TestRewriteNamedFinishReason(unittest.TestCase):
    """named tool_choice: rewrite terminal stop→tool_calls after tools seen."""

    def test_pass_through_done_and_non_data(self) -> None:
        done = "data: [DONE]\n\n"
        out, saw = _rewrite_named_finish_reason_sse(done, saw_tool_calls=False)
        self.assertEqual(out, done)
        self.assertFalse(saw)

        keep = ": keepalive\n\n"
        out, saw = _rewrite_named_finish_reason_sse(keep, saw_tool_calls=True)
        self.assertEqual(out, keep)
        self.assertTrue(saw)

    def test_malformed_json_unchanged(self) -> None:
        bad = "data: {not json}\n\n"
        out, saw = _rewrite_named_finish_reason_sse(bad, saw_tool_calls=False)
        self.assertEqual(out, bad)
        self.assertFalse(saw)

    def test_stop_without_prior_tool_calls_unchanged(self) -> None:
        line = _sse(
            {"choices": [{"delta": {"content": "hi"}, "finish_reason": "stop"}]}
        )
        out, saw = _rewrite_named_finish_reason_sse(line, saw_tool_calls=False)
        self.assertEqual(out, line)
        self.assertFalse(saw)
        self.assertEqual(_parse_sse(out)["choices"][0]["finish_reason"], "stop")

    def test_tool_calls_delta_sets_saw_flag(self) -> None:
        line = _sse(
            {
                "choices": [
                    {
    ...
    def test_stop_rewritten_after_tool_calls_seen(self) -> None:
        line = _sse(
            {
                "choices": [
        ...
        out, saw = _rewrite_named_finish_reason_sse(line, saw_tool_calls=True)
        self.assertTrue(saw)
        self.assertEqual(
            _parse_sse(out)["choices"][0]["finish_reason"], "tool_calls"
        )

    def test_same_chunk_tool_calls_and_stop_rewrites(self) -> None:
        # Terminal chunk may carry tool_calls + finish_reason together.
        line = _sse(
            {
                "choices": [
                    {
    ...
    def test_non_stop_finish_reason_unchanged(self) -> None:
        line = _sse(
            {
                "choices": [
                    {"delta": {"tool_calls": [{"index": 0}]}, "finish_reason": "length"}
                ]
            }
        )
        out, saw = _rewrite_named_finish_reason_sse(line, saw_tool_calls=False)
        self.assertTrue(saw)
        self.assertEqual(_parse_sse(out)["choices"][0]["finish_reason"], "length")
