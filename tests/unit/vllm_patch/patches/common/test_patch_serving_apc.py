...
defeat the point.  Run either way::
    ...
    pytest tests/unit/vllm_patch/patches/common/test_patch_serving_apc.py -v
    python tests/unit/vllm_patch/patches/common/test_patch_serving_apc.py
...
PATCH_PATH = (
    ...
    / "omni/vllm_patches/patches/common/patch_serving_apc.py"
)
...
def test_chat_full_relays_even_when_details_disabled():
    """P must still relay cached-token counts when details are off.

    With details off upstream writes nothing into usage, so the generator
    capture is the only source.
    """
