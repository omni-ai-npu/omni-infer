...
Run either way::
    ...
    pytest tests/unit/vllm_patch/patches/common/test_patch_health.py -v
    python  tests/unit/vllm_patch/patches/common/test_patch_health.py
"""
...
PATCH_PATH = (
    ...
    / "omni/vllm_patches/patches/common/patch_health.py"
)

HANG_SEC = 60.0

_HEALTH_TESTS_DIR = str(Path(__file__).resolve().parents[4])
if _HEALTH_TESTS_DIR not in sys.path:
    sys.path.insert(0, _HEALTH_TESTS_DIR)
...
def _install_stubs(recorder):
    ...
    def _stalled_engines(sec):
        recorder.stalled_args.append(sec)
        return recorder.stalled

    def _snapshot():
        return {0: 61.5, 1: 62.5}

    def _mark_busy():
        recorder.calls.append(("mark_busy",))

    heartbeat = _module(
        "omni_npu.diagnostics.watchdog.heartbeat",
        stalled_engines=_stalled_engines,
        snapshot=_snapshot,
        mark_busy=_mark_busy,
    )
