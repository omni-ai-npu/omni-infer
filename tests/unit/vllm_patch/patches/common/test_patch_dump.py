...
Run either way::
    ...
    pytest tests/unit/vllm_patch/patches/common/test_patch_dump.py -v
    python  tests/unit/vllm_patch/patches/common/test_patch_dump.py
"""
...
PATCH_PATH = (
    ...
    / "omni/vllm_patches/patches/common/patch_dump.py"
)

_DUMP_TESTS_DIR = str(Path(__file__).resolve().parents[4])
if _DUMP_TESTS_DIR not in sys.path:
    sys.path.insert(0, _DUMP_TESTS_DIR)
from unit.vllm_patch.patches.patch_test_utils import (  # noqa: E402
    PatchCallRecorder as _Recorder,
    StubVLLMPatch as _VLLMPatch,
    install_named_module as _module,
...
def _install_stubs(enable_dump, recorder):
    ...
    def _on_api_init(obj):
        recorder.calls.append(("on_api_init", hasattr(obj, "output_processor")))

    def _on_engine_init(obj):
        recorder.calls.append(
            ("on_engine_init", hasattr(obj, "model_executor"), hasattr(obj, "scheduler"))
        )

    def _on_worker_init(obj):
        recorder.calls.append(("on_worker_init", getattr(obj, "local_rank", None)))

    _module("vllm")
    _module("vllm.logger", init_logger=silent_logger)
    _module("vllm.v1")
    ...
    hooks = _module(
        ...
        on_api_init=_on_api_init,
        on_engine_init=_on_engine_init,
        on_worker_init=_on_worker_init,
    )
