...
PATCH_PATH = (
    ...
    / "omni/vllm_patches/patches/common/patch_repetition_detection_config.py"
)
...
def test_every_declared_attr_exists():
    ...
    for _name, target, cls in REGISTERED:
...
def test_bad_cli_json_fails_the_launch():
    ...
    try:
        parser.parse_args(["--repetition-detection", "{not json"])
...
def test_cli_rejects_semantically_invalid_config():
    ...
    except (argparse.ArgumentError, argparse.ArgumentTypeError, ValueError):
        return
...
def test_config_stays_reconstructible_for_spec_decode():
    """The default must not be parked on the instance.

    0.25.1's spec-decode path rebuilds the config through replace(), which
    raises on undeclared keys and took down every MTP worker at load_model.
    """
    _reset()
    cfg = _config_from(["--repetition-detection", GOOD_JSON])
    assert "repetition_detection" not in cfg.__dict__
...
def test_replace_stub_rejects_unknown_keys():
    """Guard for the reconstructible-config test.

    If the stub silently dropped extra keys instead of raising, that
    regression test would pass no matter what the patch does.
    """
    class _InjectedConfig(_VllmConfig):
        def __init__(self):
            super().__init__()
            self.injected_by_someone = 1
