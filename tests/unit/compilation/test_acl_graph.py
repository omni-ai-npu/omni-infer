...
reset_stale_aclgraph_resources = acl_graph_mod.reset_stale_aclgraph_resources
...
def test_reset_stale_aclgraph_resources_releases_and_rotates():
    workspace_fn = MagicMock()
    other_workspace_fn = MagicMock()
    params = GraphParams(
        task_entries={4: {"layer0": MagicMock(), "layer1": MagicMock()}},
        workspaces={
            4: {
                workspace_fn: MagicMock(),
                other_workspace_fn: MagicMock(),
    ...
    original_task_entries = params.task_entries[4]
    original_workspaces = params.workspaces[4]
    wrapper = MagicMock()
    new_pool = MagicMock()
    ...
    with patch("omni_npu.compilation.acl_graph.get_graph_params",
        ...
        reset_stale_aclgraph_resources([wrapper])

    wrapper.reset_captured_graphs.assert_called_once_with()
    assert params.task_entries[4] is original_task_entries
    assert params.workspaces[4] is original_workspaces
    assert params.task_entries[4] == {}
    assert params.workspaces[4] == {}
    mock_empty.assert_called_once_with()
    assert wrapper.graph_pool is new_pool
...
class TestACLGraphWrapper:
    ...
    def test_acl_graph_wrapper_getattr(self):
        ...
        wrapper = ACLGraphWrapper(runnable, vllm_config, CUDAGraphMode.PIECEWISE)
    ...
    def test_reset_captured_graphs(self):
        """Test reset_captured_graphs releases cached graph entries."""
        vllm_config = MagicMock(spec=VllmConfig)
        vllm_config.additional_config = {}
        wrapper = ACLGraphWrapper(MagicMock(), vllm_config, CUDAGraphMode.PIECEWISE)
        batch_descriptor = MagicMock()
        mock_graph = MagicMock()
        wrapper.concrete_aclgraph_entries = {
            batch_descriptor: ACLGraphEntry(
        ...
        mock_graph.reset.assert_called_once_with()
        entry = wrapper.concrete_aclgraph_entries[batch_descriptor]
...
class TestACLGraphWrapperUpdateMethods:
    ...
    def test_update_graph_tasks_clears_bsnd_actual_seq_lengths(self, wrapper_with_update_stream):
        """Test BSND replay updates clear actual_seq_lengths for graph task replay."""
        wrapper, forward_context, update_stream = wrapper_with_update_stream
        update_fn = MagicMock(
            return_value={
                "input_layout": "BSND",
...
def test_ensure_weak_ref_graph_params_updates_tasks_and_workspaces():
    """Test captured graph params are weak-refed in task entries and workspaces."""
    keep_tensor = torch.tensor([1.0])
    out_tensor = torch.tensor([2.0])
    workspace_tensor = torch.tensor([3.0])
    weak_keep = torch.tensor([4.0])
    weak_out = torch.tensor([5.0])
    weak_workspace = torch.tensor([6.0])
    workspace_fn = MagicMock()
    op_desc = acl_graph_mod.OpDescriptor(
        op_out_fn=MagicMock(),
        workspace_fn=workspace_fn,
        weak_ref_keys=("keep", "missing", "empty"),
    )
    task_entry = acl_graph_mod.GraphTaskEntry(
        op_desc=op_desc,
        captured_kwargs={"keep": keep_tensor, "empty": None, "other": torch.tensor([7.0])},
    ...
    graph_params = GraphParams(
        task_entries={4: {"attn": task_entry}},
        workspaces={4: {workspace_fn: [workspace_tensor]}},
    )

    def fake_weak_ref_tensors(tensors):
        if len(tensors) == 1 and tensors[0] is out_tensor:
            return [weak_out]
        if len(tensors) == 1 and tensors[0] is workspace_tensor:
    ...
    assert task_entry.out_tensors == [weak_out]
    assert task_entry.captured_kwargs["keep"] is weak_keep
    assert task_entry.captured_kwargs["empty"] is None
