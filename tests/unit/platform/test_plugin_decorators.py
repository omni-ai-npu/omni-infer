...
class TestCreateConditionalPluginDecorator:
    ...
    def test_conditional_decorator_override_return_with_custom_value(self, monkeypatch):
        """Test that pre-hook returning a non-bool/non-None value skips original and overrides return."""
        mock_plugin = Mock()
        mock_plugin.pre_check = Mock(return_value=42)
        mock_plugin.post_check = Mock()
    ...
    def test_conditional_decorator_override_return_with_dict(self, monkeypatch):
        """Test that pre-hook returning a dict overrides the return value."""
        mock_plugin = Mock()
        mock_plugin.pre_check = Mock(return_value={"memory": 1024})
        mock_plugin.post_check = Mock()
    ...
    def test_conditional_decorator_override_return_with_string(self, monkeypatch):
        """Test that pre-hook returning a string overrides the return value."""
        mock_plugin = Mock()
        mock_plugin.pre_check = Mock(return_value="overridden_value")
        mock_plugin.post_check = Mock()
    ...
    def test_conditional_decorator_false_does_not_override(self, monkeypatch):
        """Test that pre-hook returning False does not skip or override."""
        mock_plugin = Mock()
        mock_plugin.pre_check = Mock(return_value=False)
        mock_plugin.post_check = Mock()
        ...
        def mock_entry_points(group):
            if group == "test.false_no_override":
                return [mock_ep]
            return []
    ...
    def test_conditional_decorator_none_does_not_override(self, monkeypatch):
        """Test that pre-hook returning None does not skip or override."""
        mock_plugin = Mock()
        mock_plugin.pre_check = Mock(return_value=None)
        mock_plugin.post_check = Mock()
        ...
        def mock_entry_points(group):
            if group == "test.none_no_override":
                return [mock_ep]
            return []
    ...
    def test_conditional_decorator_last_plugin_override_wins(self, monkeypatch):
        """Test that when multiple plugins override, the last one's value is used."""
        mock_plugin1 = Mock()
        mock_plugin1.pre_check = Mock(return_value=100)
        mock_plugin1.post_check = Mock()

        mock_plugin2 = Mock()
        mock_plugin2.pre_check = Mock(return_value=200)
        mock_plugin2.post_check = Mock()

        mock_ep1 = Mock()
        mock_ep1.load = Mock(return_value=lambda: mock_plugin1)
        mock_ep1.name = "plugin1"

        mock_ep2 = Mock()
        mock_ep2.load = Mock(return_value=lambda: mock_plugin2)
        mock_ep2.name = "plugin2"

        def mock_entry_points(group):
            if group == "test.multi_override":
...
class TestPredefinedDecorators:
    ...
    def test_npu_model_runner_reinitialize_input_batch_is_decorated(self):
        ...
        runner_path = (
            repo_root / "omni" / "worker" / "npu_model_runner.py"
        )
...
class TestAttnDecorator:
    ...
    def test_attn_decorator_creates_correct_entry_point_group(self, monkeypatch):
        ...
        decorator = attn_decorator(attn_type='test_type')
    ...
    def test_attn_decorator_accepts_legacy_type_keyword(self, monkeypatch):
        """Test that attn_decorator still supports the legacy type keyword."""
        captured_groups = []
        ...
        decorator = attn_decorator(type='legacy')
    ...
    def test_attn_decorator_rejects_unexpected_keyword(self):
        ...
        with pytest.raises(
            TypeError,
            match="attn_decorator\\(\\) got an unexpected keyword argument 'unknown'",
        ):
            attn_decorator(attn_type='dsa', unknown='value')

    def test_attn_decorator_rejects_conflicting_type_keywords(self):
        """Test that attn_decorator rejects conflicting new and legacy names."""
        with pytest.raises(
            ValueError,
            match="attn_type and type must match when both are provided",
        ):
            attn_decorator(attn_type='dsa', type='mla')

    def test_attn_decorator_requires_type(self):
        """Test that attn_decorator requires an attention type."""
        with pytest.raises(
            TypeError,
            match="attn_decorator\\(\\) missing required argument: 'attn_type'",
        ):
            attn_decorator()
    ...
    def test_attn_decorator_with_dsa_type(self, monkeypatch):
        ...
        decorator = attn_decorator(attn_type='dsa')
    ...
    def test_attn_decorator_with_mla_type(self, monkeypatch):
        ...
        decorator = attn_decorator(attn_type='mla')
    ...
    def test_attn_decorator_with_mome_type(self, monkeypatch):
        ...
        decorator = attn_decorator(attn_type='mome')
    ...
    def test_attn_decorator_no_plugins_registered(self, monkeypatch):
        ...
        decorator = attn_decorator(attn_type='unknown')
...
class TestDetermineMemoryDecorator:
    """Tests for the determine_memory_decorator instance."""

    def setup_method(self):
        """Clear cached entry points before each test."""
        _cached_eps.clear()

    def test_determine_memory_decorator_exists(self):
        """Test that determine_memory_decorator is properly defined."""
        assert callable(determine_memory_decorator)

    def test_determine_memory_decorator_no_plugins(self, monkeypatch):
        """Test determine_memory_decorator executes original when no plugins."""
        def mock_entry_points(group):
            return []
        ...
        result = determine_available_memory("self_instance")
        assert result == 8192

    def test_determine_memory_decorator_plugin_overrides_memory(self, monkeypatch):
        """Test that a plugin can override the memory calculation."""
        mock_plugin = Mock()
        mock_plugin.pre_determine_memory = Mock(return_value=4096)
        mock_plugin.post_determine_memory = Mock()
        ...
        assert result == 4096
        assert len(func_called) == 0
        mock_plugin.post_determine_memory.assert_called_once()

    def test_determine_memory_decorator_plugin_returns_true_skips(self, monkeypatch):
        """Test that plugin returning True skips original, returns None."""
        mock_plugin = Mock()
        mock_plugin.pre_determine_memory = Mock(return_value=True)
        mock_plugin.post_determine_memory = Mock()
    ...
    def test_determine_memory_decorator_plugin_returns_false_executes_original(self, monkeypatch):
        """Test that plugin returning False allows original to execute."""
        mock_plugin = Mock()
        mock_plugin.pre_determine_memory = Mock(return_value=False)
        mock_plugin.post_determine_memory = Mock()
    ...
    def test_determine_memory_decorator_is_applied_to_npu_worker(self):
        """Test that determine_memory_decorator is applied to NPUWorker.determine_available_memory."""
        repo_root = Path(__file__).resolve().parents[3]
        worker_path = repo_root / "omni" / "worker" / "npu_worker.py"
        text = worker_path.read_text()

        assert "@determine_memory_decorator" in text
        assert "def determine_available_memory(" in text
