...
def hifloat8_module(monkeypatch):
    ...
    layers_pkg.__path__ = [str(repo_root / "src" / "omni_npu" / "layers")]
    quant_pkg = _make_module(monkeypatch, "omni_npu.layers.quantization", is_package=True)
    quant_pkg.__path__ = [str(repo_root / "src" / "omni_npu" / "layers" / "quantization")]

    _make_module(monkeypatch, "omni_npu.layers.fused_moe", is_package=True)
    ...
    mhc_pkg.__path__ = [str(repo_root / "src" / "omni_npu" / "layers" / "mhc")]
...
def test_moe_apply_experts_with_routed_experts_cv_returns_tuple(
    ...
    prepare_result = SimpleNamespace(
        ...
        shared_expert_gate_up_proj_finished_event=shared_event,
    )
