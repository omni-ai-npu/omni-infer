...
@contextmanager
def _weights_pool(vllm_config, allocator: NpuMemAllocator):
...
def _run_model_case(
    ...
    with _weights_pool(vcfg, allocator):
    ...
    # Verify memory is released across repeated sleep/wake cycles.
    free_before_sleep = torch.npu.mem_get_info()[0]
    worker.sleep(level=1)
    free_after_sleep = torch.npu.mem_get_info()[0]
    second_time_sleep_free = free_after_sleep - free_before_sleep
    bytes_per_gib = float(1024**3)
    sleep_free_delta = abs(second_time_sleep_free - first_time_sleep_free)
    assert sleep_free_delta <= 0.05 * bytes_per_gib, (
        "Sleep freed-memory delta is too large. Check for unreleased or leaked "
        "memory after weight loading: "
        f"first_time_sleep_free={first_time_sleep_free / bytes_per_gib:.4f} GiB, "
        f"second_time_sleep_free={second_time_sleep_free / bytes_per_gib:.4f} GiB, "
        f"delta={sleep_free_delta / bytes_per_gib:.4f} GiB"
    )
    time.sleep(1)
