...
def _prepare_omni_layers_namespace() -> None:
    ...
    if package_name in sys.modules:
        return
    layers_dir = Path(__file__).resolve().parents[4] / "src" / "omni_npu" / "layers"
...
def test_npu_mrope_interleaved_forward_matches_reference(npu_device):
    """NPUMRotaryEmbeddingInterleaved forward vs reference (rotary_mode=half)."""
    try:
    ...
    # mrope_section sum = rotary_dim // 2. For rotary_dim=16, [4,4] or [8,0]
    ...
    # Reset layer cache for deterministic test
    ...
    exp_q, exp_k = _apply_rotary_ref_to_qk(
        layer, query_ref, key_ref, cos, sin, apply_rotary_emb_full_dim
    )
    _assert_rotary_outputs_match(out_q, out_k, exp_q, exp_k)


def test_npu_mrope_interleaved_forward_key_none(npu_device):
    """NPUMRotaryEmbeddingInterleaved key=None."""
    try:
        from omni_npu.layers.rotary_embedding.mrope_interleaved_torch_npu import (
            NPUMRotaryEmbeddingInterleaved,
        )
    ...
    cos, sin = layer._rebuild_pos_emb(positions)
    exp_q, _ = _apply_rotary_ref_to_qk(
        layer, query_ref, query_ref, cos, sin, apply_rotary_emb_full_dim
    )
    assert out_k is None
    _assert_rotary_outputs_match(out_q, out_k, exp_q, None)