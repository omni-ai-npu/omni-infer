...
class TestNPUDSABackendReshapeNoncontiguous(unittest.TestCase):
    ...
    def test_default_branch_shapes_and_dtypes(self):
        """Default (bfloat16) branch: regression guard."""
        shapes, dtypes = self._call_with_dtype("bf16")
        self.assertEqual(shapes, ((576,), (128,)))
        self.assertEqual(dtypes, (torch.bfloat16, torch.bfloat16))
...
class TestNPUDSAMetadataBuilder(unittest.TestCase):
    ...
    def test_build_decode_sets_num_actual_tokens_from_query_start_loc(self):
        b = self._new_builder_minimal()
        block_table = torch.zeros(4, 10, dtype=torch.int32)
        seq_lens = torch.tensor([1, 2, 3, 4], dtype=torch.int32)
        query_start_loc_cpu = torch.tensor([0, 1, 3, 6, 10], dtype=torch.int32)
        query_start_loc_dev = query_start_loc_cpu.to()
        ...
        result = b._build_decode(
            ...
            query_start_loc_device=query_start_loc_dev,
            num_decode_tokens=10,
            dcp_tot_seq_lens_device=None,
        )
        self.assertEqual(result.num_actual_tokens, 10)
        self.assertTrue(torch.equal(result.query_cumlens,
                                    torch.tensor([1, 3, 6, 10], dtype=torch.int32)))
    ...
    def test_build_prefill_calls_init_cp_when_seq_parallel_enabled(self):
        ...
        b.force_first_chunk_context = True
    ...
    def test_build_decode_sets_mc2_mask_without_aligning_slot_mapping(self):
        b = self._new_builder_minimal()
        b.vllm_config.kv_transfer_config = object()
        b.vllm_config.scheduler_config.max_num_seqs = 16

        class _Decode:
            def __init__(self):
                self.mc2_mask = None
                self.num_actual_tokens = 3
