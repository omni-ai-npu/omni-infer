...
class TestNPUAttentionBackendDefaultMetadataBuilder(unittest.TestCase):
    ...
    def test_metadata_builder(self):
        ...
        self.assertTrue(meta.use_kvnz_pure_prefill)
    ...
    def test_metadata_builder_marks_prefill_with_prefix_false(self):
        ...
        self.assertFalse(meta.use_kvnz_pure_prefill)
...
class TestNPUAttentionBackendDefaultImpl(unittest.TestCase):
    ...
    def test_forward_calls_npu_fused_infer_attention_score_bsnd(self):
        ...
        with patch('torch_npu.npu_scatter_nd_update_', side_effect=fake_scatter_nd_update_), \
            patch('torch_npu.npu_fused_infer_attention_score', side_effect=fake_fused_infer_attention_score) as mock_decode:
            result = impl.forward(
                layer=layer,
                query=query,
                key=key,
            ...
            # The BSND layout (used for head_size 256) cannot process hybrid batches natively.
            # Therefore, it requires 1 call for grouped decodes (if any) + 1 call per individual prefill.
            if impl.head_size == 256:
                expected_calls = metadata.num_prefills + (1 if getattr(metadata, 'num_decode_tokens', 0) > 0 else 0)
                self.assertEqual(mock_decode.call_count, expected_calls)
            else:
                mock_decode.assert_called_once()
            self.assertIs(result, output)
    ...
    def test_forward_bsnd_all_paths_for_future_removal(self):
        ...
        with patch('torch_npu.npu_scatter_nd_update_', side_effect=fake_scatter_nd_update_), \
            ...
            # 1 decode call + 1 valid prefill call (the 0-length prefill is bypassed)
