...
class TestNPUMomeAttentionMetadataBuilder(unittest.TestCase):
    ...
    def test_init_sets_pd_disagg_and_fake_num_spec(self, mock_reorder, mock_super_init):
        ...
        self.assertFalse(builder.reuse_prefilled_tokens)
...
class TestComputePrefixCachingBlockIndices(unittest.TestCase):
    ...
    def _make_builder(self, num_spec=0, is_decode_node=False, reuse_prefilled_tokens=False):
        b = _manual_builder(reuse_prefilled_tokens=reuse_prefilled_tokens)
        b.num_spec = num_spec
    ...
    def test_decode_node_recompute_skipped_when_reuse_prefilled_tokens(self):
        """OMNI_REUSE_PREFILLED_TOKENS disables decode recompute block override."""
        b = self._make_builder(num_spec=2, is_decode_node=True, reuse_prefilled_tokens=True)
        common = self._make_common(num_computed=[8], seq_lens=[12])
        num_accepted = torch.tensor([2], dtype=torch.int32)
        num_prompt = torch.tensor([10], dtype=torch.int32)
