...
def test_compute_probs_and_sample_non_greedy_calls_npu(rejection_mod, monkeypatch):
    ...
    monkeypatch.setattr(
        mod.model_extra_config.operator_opt_config,
        "use_topk_topp_stream",
        True,
    )
    called = {}
...
def test_compute_probs_and_sample_skips_stream_wait_when_flag_disabled(rejection_mod, monkeypatch):
    """use_topk_topp_stream=False bypasses stream context but still drives the NPU op."""
    mod, fake_torch_npu, fake_default_stream = rejection_mod
    monkeypatch.setattr(
        mod.model_extra_config.operator_opt_config,
        "use_topk_topp_stream",
    ...
    fake_torch_npu.npu_top_k_top_p_sample = (
        lambda logits, top_k, top_p, q, is_need_logits: (
    ...
    assert stream not in fake_default_stream.waited_streams
    assert torch.equal(token_ids, torch.tensor([1, 0], dtype=torch.int32))


def test_compute_probs_and_sample_disable_npu_top_k_top_p_path(rejection_mod, monkeypatch):
    """disable_npu_top_k_top_p_sample=True must skip the NPU op and use the softmax fallback."""
    mod, fake_torch_npu, _ = rejection_mod
    monkeypatch.setattr(
        mod.model_extra_config.operator_opt_config,
        "disable_npu_top_k_top_p_sample",
        True,
    )
    monkeypatch.setattr(
        mod,
    ...
    fake_logits = torch.tensor([[2.0, 1.0], [1.0, 2.0]], dtype=torch.bfloat16)
    monkeypatch.setattr(mod, "apply_top_k_top_p", lambda logits, top_k, top_p: fake_logits)

    npu_calls = {"count": 0}
    def fake_npu_sample(*args, **kwargs):
        npu_calls["count"] += 1
        return torch.tensor([0, 0], dtype=torch.int32), fake_logits
    fake_torch_npu.npu_top_k_top_p_sample = fake_npu_sample
    ...
    token_ids, out_logits = mod.compute_probs_and_sample(
        ...
        metadata=metadata,
        stream=object(),
        use_npu_sample=False,
    )

    assert npu_calls["count"] == 0
    assert token_ids.dtype == torch.int32
    assert token_ids.shape == (2,)
    assert torch.equal(out_logits, fake_logits)


def test_rejection_sample_waits_stream_when_flag_enabled(rejection_mod, monkeypatch):
    """use_topk_topp_stream=True drives rejection_sample to wait on the side stream."""
    ...
    assert stream in fake_default_stream.waited_streams


def test_sample_recovered_tokens_stream_branches(rejection_mod, monkeypatch):
    """sample_recovered_tokens waits the side stream only when use_topk_topp_stream=True."""
    mod, _, fake_default_stream = rejection_mod
    # The native helper has its own dedicated tests; stub it out so this test
    # focuses on the use_topk_topp_stream gate.
    monkeypatch.setattr(mod, "sample_recovered_tokens_native", lambda *a, **k: None)
    ...
    common_kwargs = dict(
        ...
        target_probs=torch.tensor(
            [[0.2, 0.3, 0.5], [0.4, 0.6, 0.0]], dtype=torch.float32
        ),
    ...
    stream_on = object()
    mod.sample_recovered_tokens(stream=stream_on, **common_kwargs)
    assert stream_on in fake_default_stream.waited_streams

    snapshot = list(fake_default_stream.waited_streams)
    monkeypatch.setattr(
        mod.model_extra_config.operator_opt_config,
        "use_topk_topp_stream",
        False,
    ...
    stream_off = object()
    mod.sample_recovered_tokens(stream=stream_off, **common_kwargs)
    assert stream_off not in fake_default_stream.waited_streams
    assert fake_default_stream.waited_streams == snapshot
...
def _build_npu_rejection_sampler(mod, monkeypatch):
...
def _patch_npu_rejection_forward_deps(mod, rejection, monkeypatch, output_ids):
    ...
    monkeypatch.setattr(
        ...
        lambda target_logits, cu, sm, md, stream, use_npu_sample: (
