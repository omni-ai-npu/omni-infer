...
def test_generate_coins_waits_stream_and_supports_seeded_generators(topk_mod, monkeypatch):
...
def test_generate_coins_skips_stream_wait_when_flag_disabled(topk_mod, monkeypatch):
    """When use_topk_topp_stream=False, no stream context is entered and wait_stream is never called."""
    mod, _, fake_default_stream = topk_mod
    monkeypatch.setattr(
        mod.model_extra_config.operator_opt_config,
        "use_topk_topp_stream",
    ...
    g0 = torch.Generator(device=probs.device.type).manual_seed(11)
    g1 = torch.Generator(device=probs.device.type).manual_seed(22)
    q = mod.generate_coins(probs, {0: g0, 1: g1}, stream)
...
def test_generate_coins_no_generators_broadcast_path(topk_mod, monkeypatch):
    """Empty generators dict triggers the broadcast q.exponential_() path in both branches."""
    mod, _, _ = topk_mod
    monkeypatch.setattr(
        mod.model_extra_config.operator_opt_config,
        "use_topk_topp_stream",
    ...
    probs = torch.zeros((3, 5), dtype=torch.bfloat16)
    q = mod.generate_coins(probs, {}, object())
    assert q.shape == probs.shape
    assert q.dtype == torch.float32
    # exponential_() guarantees strictly positive values.
...
def test_random_sample_basic(topk_mod, monkeypatch):
    ...
    stream = object()
    result = mod.random_sample(probs.clone(), idx=None, generators={}, stream=stream)
...
def test_random_sample_with_generators(topk_mod):
    ...
    result1 = mod.random_sample(probs.clone(), idx=None, generators={0: g0, 1: g1}, stream=stream)
    result2 = mod.random_sample(probs.clone(), idx=None, generators={0: g0b, 1: g1b}, stream=object())
...
def test_random_sample_with_idx(topk_mod):
    ...
    r1 = mod.random_sample(probs.clone(), idx=None, generators={}, stream=object())
    torch.manual_seed(42)
    torch_npu.npu.manual_seed(42)
    r2 = mod.random_sample(probs.clone(), idx=idx, generators={}, stream=object())
    ...
    result = mod.random_sample(probs2.clone(), idx=idx2, generators={}, stream=object())
...
def test_random_sample_skips_stream_wait_when_flag_disabled(topk_mod, monkeypatch):
    """When use_topk_topp_stream=False, random_sample does not wait on the side stream."""
    mod, _, fake_default_stream = topk_mod
    monkeypatch.setattr(
        mod.model_extra_config.operator_opt_config,
        "use_topk_topp_stream",
...
def test_random_sample_branches_produce_equivalent_outputs_under_seed(topk_mod, monkeypatch):
    """Stream-on and stream-off branches yield identical outputs when seeded identically."""
    mod, _, _ = topk_mod
    probs = torch.tensor([[0.1, 0.7, 0.2], [0.3, 0.1, 0.6]], dtype=torch.float32)
    ...
    g0 = torch.Generator(device=probs.device.type).manual_seed(7)
    g1 = torch.Generator(device=probs.device.type).manual_seed(13)
    r_on = mod.random_sample(probs.clone(), idx=None, generators={0: g0, 1: g1}, stream=object())
    ...
    g0 = torch.Generator(device=probs.device.type).manual_seed(7)
    g1 = torch.Generator(device=probs.device.type).manual_seed(13)
    r_off = mod.random_sample(probs.clone(), idx=None, generators={0: g0, 1: g1}, stream=object())

    assert torch.equal(r_on, r_off)
