...
"""
Add HYBRID_ATTN_GROUP_SIZE environment variable support to override group size.
"""
...
def _get_kv_cache_groups_uniform_page_size_patched(kv_cache_spec: dict[str, KVCacheSpec]):
    """
    ...
    if (override_group_size := int(os.getenv("HYBRID_ATTN_GROUP_SIZE", "0"))) > 0:
        logger.warning(f"Overriding hybrid attention group size from {group_size} to {override_group_size}.")
...
class OverrideGroupSizePatch(VLLMPatch):
    ...
    # Patch start
    _get_kv_cache_groups_uniform_page_size = _get_kv_cache_groups_uniform_page_size_patched
    # patch end
