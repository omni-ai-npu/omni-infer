...
class RoutedExpertsReaderPDSupportPatch(VLLMPatch):
    ...
    def attach_buffer(
        ...
        if hf_config.num_experts_per_tok > torch.iinfo(torch.uint8).max:
            host_dtype = np.uint16
        else:
            host_dtype = np.uint8
        shape = (
            max_num_kv_tokens,
...
class RoutedExpertsCapturerTPAggregatePatch(VLLMPatch):
    ...
    def init_buffer(
        ...
        if num_experts_per_tok > torch.iinfo(torch.uint8).max:
            device_dtype = torch.uint16
            host_dtype = np.uint16
        else:
    ...
    def save_captured_experts(self, indices: np.ndarray) -> None:
        ...
        if tp_rank == 0:
            ...
            dist.gather(
                local_data,
                gather_list=all_data,
                dst=tp_group.first_rank,
                group=tp_group.device_group,
            )
            dist.gather(
                real_mapping,
                gather_list=all_mapping,
                dst=tp_group.first_rank,
                group=tp_group.device_group,
            )
        else:
...
def _slice_first_logprobs(logprobs: Any) -> Any:
    return logprobs[:1] if logprobs else logprobs


def _first_token_cumulative_logprob(logprobs: Any, token_id: int,
                                    fallback: Any) -> Any:
...
def _stash_prefilled_token_for_completion(serving: Any, request_output: Any) -> None:
    if not _is_prefill_node(serving):
        return
    if os.getenv("OMNI_REUSE_PREFILLED_TOKENS", "0") != "1":
        return
    kv_params = getattr(request_output, "kv_transfer_params", None)
    if not kv_params:
        return
    outputs = getattr(request_output, "outputs", None) or []
    if not outputs:
    ...
    token_ids = getattr(first_output, "token_ids", None) or []
    if not token_ids:
        return
    prefilled_token_id = token_ids[0]
    prefilled_logprobs = _slice_first_logprobs(
        getattr(first_output, "logprobs", None))
    kv_params["prefilled_token"] = [prefilled_token_id]
    kv_params["stop_reasons"] = [
        getattr(output, "stop_reason", None) for output in outputs
    ]
    kv_params["prefilled_logprobs"] = prefilled_logprobs
    kv_params["prefilled_cumulative_logprob"] = _first_token_cumulative_logprob(
        prefilled_logprobs,
        prefilled_token_id,
        getattr(first_output, "cumulative_logprob", None),
    )
...
class ExpertIdServingCompletionFinal(VLLMPatch):
    ...
    def request_output_to_completion_response(
        ...
        for result in final_res_batch:
            ...
            _stash_prefilled_token_for_completion(self, result)
