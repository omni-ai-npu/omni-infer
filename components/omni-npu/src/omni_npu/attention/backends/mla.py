...
class NPUMLAMetadataBuilder(MLACommonMetadataBuilder[NPUMLAMetadata]):
    ...
    def build(
        ...
        if self._use_fi_prefill and num_prefills > 0:
            assert isinstance(metadata.prefill, FlashInferPrefillMetadata)
            self._build_fi_prefill_wrappers(metadata.prefill)
        ...
        if metadata.decode is not None:
            ...
            if hasattr(self, "sink_len") and self.sink_len > 0:
                ...
                assert not model_extra_config.operator_opt_config.use_aicpu_fa_tiling
                metadata.decode.sink_len = self.sink_len
                metadata.decode.seq_lens = [
                    self.sink_len if seq == 0 else seq for seq in metadata.decode.seq_lens
                ]
        ...
        if metadata.prefill is not None:
            ...
            if hasattr(self, "sink_len") and self.sink_len > 0:
                ...
                metadata.prefill.seq_lens = [
                    self.sink_len if seq == 0 else seq for seq in metadata.prefill.seq_lens
                ]
        ...
        metadata.get_slot_mapping_2d = self._lazy_slot_mapping_2d(metadata.slot_mapping)

        return metadata

    def _lazy_slot_mapping_2d(self, slots):
        return lambda: torch.stack([
            slots // self.kv_cache_spec.block_size,
            slots % self.kv_cache_spec.block_size,
        ], dim=-1)
