...
class NPUMomeAttentionMetadataBuilder(GDNAttentionMetadataBuilder):
    ...
    def __init__(
        ...
        self.enable_flashcomm2 = (
            model_extra_config.parall_config.enable_flashcomm2 and
    ...
    def _build_for_flashcomm2(
        ...
        token_range = [
            [min(t * padded_local_size, num_actual_tokens), 
             min((t + 1) * padded_local_size, num_actual_tokens)]
        ...
        if local_size[tp_rank] > 0:
            ...
            num_computed_tokens_local = \
                common_attn_metadata.num_computed_tokens_cpu[req_idx_start[tp_rank]:req_idx_end[tp_rank]].clone()
    ...
    def build(
        ...
        if (
            ...
            cache_indices = self.cache_indices_tensor[:num_decodes]
