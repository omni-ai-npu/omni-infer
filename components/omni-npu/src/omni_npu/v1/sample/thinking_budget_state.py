...
def _normalize_thinking_token_budget(value: Any) -> int | None:
    if value is None:
        return None
...
def _get_thinking_token_budget(params: Any) -> int | None:
    extra_args = getattr(params, "extra_args", None)
    if extra_args is None:
        return None
    return _normalize_thinking_token_budget(
        extra_args.get("thinking_token_budget")
    )
...
class ThinkingBudgetStateHolder:
    ...
    def __init__(
        ...
        logger.info(f"ThinkingBudgetStateHolder reasoning_config :{reasoning_config}")
        if reasoning_config is None:
    ...
    def sync_batch(self, batch_update: BatchUpdate | None) -> None:
        ...
        for index, params, prompt_tok_ids, output_tok_ids in batch_update.added:
            thinking_token_budget = _get_thinking_token_budget(params)
