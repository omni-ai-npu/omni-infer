...
class ReasoningConfig:
    ...
    ban_tool_call_in_thinking: bool = False
    ...
    def tool_call_start_token_id(self) -> int | None:
        """Token id used by ``ban_tool_call_in_thinking``; ``None`` when the
        ban is off or the tokenizer has neither ``<|tool_call_start|>`` nor
        the ``[unused11]`` fallback."""
    ...
    def initialize_token_ids(self, model_config: ModelConfig) -> None:
        ...
        # Optional: resolve the ``<|tool_call_start|>`` id needed by
        # ``ban_tool_call_in_thinking``. Falls back to ``[unused11]`` (the
        # legacy Pangu key, see ``PanguToolParser``). Silent on miss — the
        # ban factory will simply return ``None`` and the feature stays off.
        if self.ban_tool_call_in_thinking:
