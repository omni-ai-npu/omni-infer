...
class NPUPanguSparseAttention(torch.nn.Module):
    ...
    def _apply_MOME(
        ...
        if enable_sequence_parallel:
            ...
            gathered_cache = tp_group.all_gather(cache_for_sp[:max_local_reqs], dim=0).view(self.tp_size, max_local_reqs, state_len, dim)
            cache_list = []
            for t in range(self.tp_size):
                cache_list.append(gathered_cache[t, :fc2_metadata.num_valid_cache[t]]) # (num_valid_cache, state_len, dim)
