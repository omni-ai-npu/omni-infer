...
class NPUPanguSparseAttention(torch.nn.Module):
    ...
    def _forward_dummy(
        ...
        hidden_states = self.o_proj(attn_output)[0]
    ...
    def _apply_MOME(
        ...
        attn_metadata: MLACommonMetadata, # required by omni-cache
        ...
        kv_idx = self.conv_index[layer]
        cache = self.mome_attn.kv_cache[get_forward_context().virtual_engine][kv_idx]

        if not self.on_ascend950:
            if ena_sp:
                init_idx, save_idx, meta = mome_metadata.conv_sp_meta
                return conv_sp(x, layer.weight, cache, init_idx, save_idx, meta, inplace)

            x = torch.ops.vllm.npu_pangu_mome_conv(
                x, layer.weight, cache,
            ...
            x = layer.forward_prefill(
                x, 
                cache[:, -(self.mome_kernel_width - 1):], 
    ...
    def _apply_SWA_attention_prefill_absorb(
        ...
        attn_output = (
            torch_npu.npu_transpose_batchmatmul(attn_output, self.W_UV, perm_y=(1, 0, 2))
                .reshape(-1, self.num_local_heads * self.v_head_dim)
            )
    ...
    def _mla_prolog(
        ...
        if self.cache_config.cache_dtype in ["hif8_ds_mla", "fp8_ds_mla"] \
            and decode_only(attn_metadata) \
            and not self.is_prefill_node:
    ...
    def _kv_down_mome(self, hidden_states, attn_metadata, mome_metadata):
        ...
        if self.use_mome:
            if self.use_mome_inplace_update:
                conv_states = self.mome_attn.kv_cache[get_forward_context().virtual_engine][1]
            ...
            with torch.npu.npugraph_ex.scope.limit_core_num(8, 24):
                ...
                if self.indexer.use_rope_fusion_op:
                    ...
                    q_pe_i, q_nope_i = torch.split(
                        indexer_q,
                        [self.indexer.qk_rope_head_dim, self.indexer.index_head_dim - self.indexer.qk_rope_head_dim],
                        dim=-1,
                    )
    ...
    def _mla_epilog(
        ...
        if self.use_mome:
            ...
            if self.disable_o_conv_tp and self.o_proj.tp_size > 1:
...
def npu_pangu_forward(
    ...
    if attn_metadata is None:
        ...
        if has_decode and has_prefill:
            ...
            if enable_cp:
                assert self.moe_comm_strategy != "allreduce", "Context parallel is not supported with allreduce MoE communication strategy"
