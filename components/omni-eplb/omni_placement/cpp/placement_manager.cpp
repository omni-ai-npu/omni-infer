...
Placement::Placement(int rank, int world_size, int hccl_comm_world_size,
                     ...
                     bool enable_dynamic, bool enable_new_context)
    : rank_(rank), world_size_(world_size),
      ...
      enable_new_context_(enable_new_context) {
...
void Placement::placement_manager(int32_t deviceId,
    ...
    if (enable_new_context_) {
        ...
        std::cout << "[EPLB-INFO] New Context Created on Device: "
                  << deviceId << std::endl;
    } else {
...
void Placement::start_thread() {
    if (!worker_thread_.joinable()) {
        ...
        std::cout << getTimestamp()
                  << "[EPLB-Info] EPLB Worker thread created. "
                  << "Device ID: " << deviceId
                  << std::endl;
    }
}
...
PYBIND11_MODULE(omni_placement, m) {
    ...
    py::class_<Placement>(m, "Placement")
        ...
        .def(py::init<int, int, int, int, ClusterActivation *,
                      PlacementMapping *, char *, bool, bool>(),
             py::arg("rank"), py::arg("world_size"),
             ...
             py::arg("enable_new_context") = false)
