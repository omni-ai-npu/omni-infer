...
run_cpp_tests() {
    ...
    CMAKE_OPTIONS+=(-DCMAKE_CXX_FLAGS="-fprofile-arcs -ftest-coverage")
    CMAKE_OPTIONS+=(-DCMAKE_C_FLAGS="-fprofile-arcs -ftest-coverage")
    CMAKE_OPTIONS+=(-DCMAKE_EXE_LINKER_FLAGS="-lgcov --coverage")
    if check_npu_presence; then
        CMAKE_OPTIONS+=(-Denable_tests_with_npu=ON)
        echo "NPU detected. Unit tests with NPU will be built."
    else
        echo "NPU not detected. Only CPU-based unit tests will be built."
    fi

    echo "Configuring the project with cmake... Options: ${CMAKE_OPTIONS[*]} .."
    cmake "${CMAKE_OPTIONS[@]}" .. || {
    ...
    # --- Generate C++ Coverage Report with gcovr ---
    if command -v gcovr >/dev/null 2>&1; then
        ...
        --print-summary \
        . || {
            echo -e "\033[33mWarning: gcovr failed, but .gcda files exist.\033[0m"
        }
        echo -e "\033[32mCoverage report: $(pwd)/coverage_report/index.html\033[0m"
    else
        echo -e "\033[33mNote: Install 'gcovr' for HTML coverage: pip3 install gcovr\033[0m"
    fi
    safe_cd ..
}
...
safe_cd "$START_DIR"
echo -e "\033[32mTest process finished.\033[0m"