...
: "${ENABLE_CHUNK_PREFILL:=0}"
: "${MAX_BATCHED_TOKENS:=}"   # empty = use default per-side logic
: "${CHUNKED_PREFILL_TOKEN_THRESHOLD:=0}"

# ─── Cluster / networking ─────────────────────────────────────────────────
: "${TP_NNODES:=1}"
: "${ASCEND_GLOBAL_LOG_LEVEL:=3}"# base.sh — default configuration for Pangu V2 hybrid PD deployment.
# Sourced by launch_prefill.sh / launch_decode.sh.
# Every value can be overridden via environment variable or a profile config.

# ─── Serving ──────────────────────────────────────────────────────────────
# MODEL_PATH and SERVED_MODEL_NAME are configured in launch_pd.sh,
# exported into the container before this script runs.
: "${BSZ:=8}"
: "${MAX_LEN:=8192}"
: "${DIST_BACKEND:=mp}"
: "${ENFORCE_EAGER:=1}"
