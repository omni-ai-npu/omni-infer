...
#
# launch_prefill.sh — bring up one prefill instance of a Pangu V2 hybrid
# 3P-1D deployment.
#
# In 3P-1D mode there are 3 prefill instances (kv_rank 0..2) each producing
# KV for a single decode instance.  Set PREFILL_RANK to identify this
# instance (default 0).
#
...
#
# Usage:
#     PREFILL_RANK=0 bash launch_prefill.sh                  # first prefill
#     PREFILL_RANK=1 bash launch_prefill.sh                  # second prefill
#     PREFILL_RANK=2 bash launch_prefill.sh                  # third prefill
...
# ─── PYTHONHASHSEED must match the proxy for APC block-hash matching ─────
: "${PYTHONHASHSEED:=123}"
...
: "${OMNI_CACHE_MMAP_FILE:=omni_cache_p${PREFILL_RANK}}"
...
: "${LOG_DIR:=$SCRIPT_DIR/logs/prefill_${PREFILL_RANK}}"
...
if [[ "${ENABLE_OMNI_CACHE}" == "1" ]]; then
    ...
    KV_PARALLEL_SIZE=$((NUM_PREFILL_INSTANCES + DECODE_DP_SIZE))
else
...
echo "[launch_prefill] prefill_rank=$PREFILL_RANK / $NUM_PREFILL_INSTANCES"
...
echo "${VLLM_CMD[@]}" > "$LOG_DIR/vllm_cmd.log"
echo "[launch_prefill] vllm command saved to: $LOG_DIR/vllm_cmd.log"
