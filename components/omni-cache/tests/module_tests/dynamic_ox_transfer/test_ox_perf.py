...
def parse_cluster_count(shard_list_str):
    """Parse cluster count from --shard-list.

    Semicolons separate P clusters. Each request should choose one cluster_id
    from [0, cluster_count).
    """
    if not shard_list_str:
        return 1
    clusters = [c.strip() for c in shard_list_str.split(";") if c.strip()]
...
def _build_request(request_id, src_ids, dst_ids, cluster_count):
    cluster_id = random.randrange(cluster_count)
...
def _warmup(socket, num_requests, blocks_per_request, num_blocks, cluster_count):
    ...
    for i in range(num_requests):
        data = _build_request(f"warmup_{i}", src_ids, dst_ids, cluster_count)
        socket.send(data)
...
def _run_sequential(socket, num_requests, blocks_per_request, num_blocks, cluster_count):
    ...
    for i in range(num_requests):
        data = _build_request(f"seq_{i}", src_ids, dst_ids, cluster_count)
...
def _run_pipeline(socket, num_requests, blocks_per_request, num_blocks, cluster_count):
    ...
    for i in range(num_requests):
        rid = f"pipe_{i}"
        data = _build_request(rid, src_ids, dst_ids, cluster_count)
...
def run_benchmark(args, block_tp_size, tp_size):
    ...
    cluster_count = parse_cluster_count(args.shard_list)
    print(f"[UT] cluster_count={cluster_count}; request cluster_id is randomly selected per request")

    # Warmup
    _warmup(
        sock, args.warmup_requests, args.blocks_per_request,
        args.num_blocks, cluster_count)
    ...
    if args.pipeline:
        latencies, t_start, t_end = _run_pipeline(
            sock, args.num_requests, args.blocks_per_request,
            args.num_blocks, cluster_count)
...
def main():
    ...
    p.add_argument("--ox-path", default=None,
                   help="Path to the OX binary (default: repository OX binary)")
