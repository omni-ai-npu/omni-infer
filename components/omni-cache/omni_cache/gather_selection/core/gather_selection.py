...
def _resolve_gs_table_status(
    ...
    if int(os.getenv("USE_OMNI_INPUT_BATCH", "0")):
        ...
        if (
            input_batch is None
            or not hasattr(input_batch, "has_gs_buffers")
            or not input_batch.has_gs_buffers()
        ):
            raise RuntimeError(
                "GatherSelection buffers are not available on OmniCacheInputBatch"
            )
        return (
...
def dsa_gather_selection(
    ...
    selection_kv_block_table_npu, selection_kv_block_status_npu = (
        _resolve_gs_table_status(omni_cache, layer_idx, bs)
    )
    ...
    omni_cache.reuse_rate[layer_idx] = (
        omni_cache.reuse_rate[layer_idx] * omni_cache.record_smooth_alpha
        + reusedNum.sum() / total_blocks.sum()
        * (1 - omni_cache.record_smooth_alpha)
    )
