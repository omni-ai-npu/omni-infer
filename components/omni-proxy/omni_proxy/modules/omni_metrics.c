...
ngx_str_t omni_health_status_export_json(omni_global_state_t *gs, ngx_pool_t *pool) {
    ...
    for (i = 0, cnt = 0; i < MAX_ENCODE_UPSTREAMS && cnt < gs->num_encode_endpoints; i++) {
        ...
        cnt++;
        required_size += 300;
        required_size += gs->encode_states[i].comm.address.name_str.len;
    }
    ...
    ngx_uint_t healthy_encode_count = 0;
    for (i = 0, cnt = 0; i < MAX_ENCODE_UPSTREAMS && cnt < gs->num_encode_endpoints; i++) {
        if (gs->encode_states[i].comm.status != STATUS_ENABLE) {
            continue;
        }
        cnt++;
        if (gs->encode_states[i].healthy) {
            healthy_encode_count++;
        }
    }
    ...
    ngx_uint_t num_encode_instance = gs->num_encode_endpoints;
    ...
    ngx_uint_t total_upstreams = gs->num_prefill_endpoints + gs->num_decode_endpoints + gs->num_encode_endpoints;
    ngx_uint_t total_healthy = healthy_prefill_count + healthy_decode_count + healthy_encode_count;
    ngx_uint_t require_encode = gs->num_encode_endpoints > 0;

    if (healthy_prefill_count == 0 || healthy_decode_count == 0 || (require_encode && healthy_encode_count == 0)) {
        code = 503;
    ...
    p = ngx_snprintf(p, end - p, "    \"summary\": \"%uiE%uiP%ui-%uiD%ui\",\n", num_encode_instance, num_prefill_instance, num_prefill_card_per_instance, num_decode_instance, num_decode_card_per_instance);
    p = ngx_snprintf(p, end - p, "    \"total_prefill_servers\": %d,\n", gs->num_prefill_endpoints);
    p = ngx_snprintf(p, end - p, "    \"health_prefill_servers\": %d,\n", healthy_prefill_count);
    p = ngx_snprintf(p, end - p, "    \"total_encode_servers\": %d,\n", gs->num_encode_endpoints);
    p = ngx_snprintf(p, end - p, "    \"health_encode_servers\": %d,\n", healthy_encode_count);
    p = ngx_snprintf(p, end - p, "    \"total_decode_servers\": %d,\n", gs->num_decode_endpoints);
    p = ngx_snprintf(p, end - p, "    \"health_decode_servers\": %d,\n", healthy_decode_count);
    p = ngx_snprintf(p, end - p, "    \"encode\": [\n");
