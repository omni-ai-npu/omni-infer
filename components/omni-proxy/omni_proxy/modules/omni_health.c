...
static void omni_health_check_finalize(ngx_connection_t *c, ngx_int_t is_healthy) {
    ...
    const char* type_str = (ctx->upstream_type == OMNI_UPSTREAM_TYPE_PREFILL) ? "Prefill" :
                           (ctx->upstream_type == OMNI_UPSTREAM_TYPE_DECODE) ? "Decode" : "Encode";

    if (ctx->upstream_type == OMNI_UPSTREAM_TYPE_PREFILL) {
        healthy_flag = &gs->prefill_states[ctx->upstream_index].healthy;
    } else if (ctx->upstream_type == OMNI_UPSTREAM_TYPE_DECODE) {
        healthy_flag = &gs->decode_states[ctx->upstream_index].healthy;
    } else {
        healthy_flag = &gs->encode_states[ctx->upstream_index].healthy;
    }
...
void omni_run_single_health_check(omni_health_check_job_t *job, ngx_log_t *log, ngx_uint_t upstream_type, ngx_uint_t index)
{
    ...
    if (upstream_type == OMNI_UPSTREAM_TYPE_PREFILL) {
        addr = &gs->prefill_states[index].comm.address;
    } else if (upstream_type == OMNI_UPSTREAM_TYPE_DECODE) {
        addr = &gs->decode_states[index].comm.address;
    } else {
        addr = &gs->encode_states[index].comm.address;
    }
