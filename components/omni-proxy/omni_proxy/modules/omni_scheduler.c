...
{
    if (gs->num_encode_endpoints == 0) {
        return NGX_DECLINED;
    }

    uint32_t selected = UINT32_MAX;
    uint32_t least_load = UINT32_MAX;
    uint32_t least_running = UINT32_MAX;
    uint32_t cnt = 0;

    for (uint32_t m = gs->last_selected_encode;
         m < MAX_ENCODE_UPSTREAMS + gs->last_selected_encode && cnt < gs->num_encode_endpoints; m++) {
        uint32_t j = m % MAX_ENCODE_UPSTREAMS;
        omni_upstream_encode_t *encode = &gs->encode_states[j];
        if (encode->comm.status != STATUS_ENABLE) {
            if (encode->comm.status == STATUS_UNUSED) {
                m = MAX_ENCODE_UPSTREAMS - 1;
            }
            continue;
        }
        cnt++;
        uint32_t load_tokens = encode->num_tokens;
        uint32_t running = encode->num_running;
        if (load_tokens < least_load || (load_tokens == least_load && running < least_running)) {
            least_load = load_tokens;
            least_running = running;
            selected = j;
            if (least_load == 0 && least_running == 0) {
                break;
            }
        }
    }
    ...
    req->encode_upstream_endpoint_idx = (uint16_t)selected;
    gs->last_selected_encode = (uint16_t)(selected + 1);
    req->has_encode_sched = true;
    uint32_t encode_items = (req->encode_item_count > 0) ? req->encode_item_count : ctx->encoder_item_count;
    if (encode_items == 0) {
        encode_items = 1;
    }
    ngx_atomic_fetch_add(&gs->encode_states[selected].num_running, 1);
    ngx_atomic_fetch_add(&gs->encode_states[selected].num_tokens, encode_items);
    ngx_atomic_fetch_add(&gs->encode_states[selected].comm.ref, 1);

    struct timeval tv;
    omni_get_current_time(&tv);
    req->metrics.time_encode_scheduled = tv;

    ngx_log_error(NGX_LOG_INFO, omni_get_http_request(req)->connection->log, 0,
                  "[Encoder-%d] Schedule to: %d (items=%ui, load=%ui)",
                  req->slot_index,
                  req->encode_upstream_endpoint_idx,
                  encode_items,
                  gs->encode_states[selected].num_tokens);
